...
#if defined(DEBUG) && defined(DUMP_IONE_CLOCKS) && defined(__linux__)

#pragma GCC diagnostic ignored "-Wstrict-prototypes"
#include <mach/clock.h>

/*#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,29))
#include <../mach-ione2/clock_34xx.h>
#define ONCHIP_CLKS onchip_clks
#else
#include <../mach-ione2/clock34xx.h>
#define ONCHIP_CLKS onchip_34xx_clks
#endif

static void ione3_clk_recalc(struct clk *clk) {}
static void ione3_followparent_recalc(struct clk *clk) {}
static void ione3_propagate_rate(struct clk *clk) {}
static void ione3_table_recalc(struct clk *clk) {}
static long ione3_round_to_table_rate(struct clk *clk, unsigned long rate) { return 0; }
static int ione3_select_table_rate(struct clk *clk, unsigned long rate) { return 0; }

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,29))
static void ione3_dpll_recalc(struct clk *clk, unsigned long parent_rate,
							  u8 rate_storage) {}
static void ione3_clkoutx2_recalc(struct clk *clk, unsigned long parent_rate,
								  u8 rate_storage) {}
static void ione3_dpll_allow_idle(struct clk *clk) {}
static void ione3_dpll_deny_idle(struct clk *clk) {}
static u32 ione3_dpll_autoidle_read(struct clk *clk) { return 0; }
static int ione3_noncore_dpll_enable(struct clk *clk) { return 0; }
static void ione3_noncore_dpll_disable(struct clk *clk) {}
static int ione3_noncore_dpll_set_rate(struct clk *clk, unsigned long rate) { return 0; }
static int ione3_core_dpll_m2_set_rate(struct clk *clk, unsigned long rate) { return 0; }
void followparent_recalc(struct clk *clk, unsigned long new_parent_rate,
								u8 rate_storage) {}
long ione2_dpll_round_rate(struct clk *clk, unsigned long target_rate) { return 0; }
void ione2_clksel_recalc(struct clk *clk, unsigned long new_parent_rate,
								u8 rate_storage) {}
long ione2_clksel_round_rate(struct clk *clk, unsigned long target_rate) { return 0; }
int ione2_clksel_set_rate(struct clk *clk, unsigned long rate) { return 0; }
void ione2_fixed_divisor_recalc(struct clk *clk, unsigned long new_parent_rate,
									   u8 rate_storage) {}
void ione2_init_clksel_parent(struct clk *clk) {}
#endif*/

static void dump_ione_clocks(void)
{
/*	struct clk **c;
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,29))
	...
	ione3_dpll_allow_idle(0);
	ione3_dpll_deny_idle(0);
	ione3_dpll_autoidle_read(0);
	ione3_clk_recalc(0);
	ione3_followparent_recalc(0);
	ione3_propagate_rate(0);
	ione3_table_recalc(0);
	ione3_round_to_table_rate(0, 0);
	ione3_select_table_rate(0, 0);
#endif
...
static SGX_INLINE void dump_ione_clocks(void) {}

#endif

#if defined(SGX_OCP_REGS_ENABLED)

#define SYS_IONE_OCP_REGS_SYS_PHYS_BASE		(SYS_IONE_SGX_REGS_SYS_PHYS_BASE + EUR_CR_OCP_REVISION)
#define SYS_IONE_OCP_REGS_SIZE				0x110
...
static SGX_INLINE PVRSRV_ERROR EnableSGXClocksWrap(SYS_DATA *psSysData)
{
...
static SGX_INLINE PVRSRV_ERROR EnableSystemClocksWrap(SYS_DATA *psSysData)
{
...
static PVRSRV_ERROR SysLocateDevices(SYS_DATA *psSysData)
{
	...
	eError = OSBaseAllocContigMemory(SYS_IONE_SGX_REGS_SIZE,
									 &gsSGXRegsCPUVAddr,
									 &sCpuPAddr);
	if(eError != PVRSRV_OK)
	{
	...
	gsSGXDeviceMap.ui32RegsSize = SYS_IONE_SGX_REGS_SIZE;
	...
	OSMemSet(gsSGXRegsCPUVAddr, 0, SYS_IONE_SGX_REGS_SIZE);
	...
	gsSGXDeviceMap.sRegsSysPBase.uiAddr = SYS_IONE_SGX_REGS_SYS_PHYS_BASE;
	gsSGXDeviceMap.sRegsCpuPBase = SysSysPAddrToCpuPAddr(gsSGXDeviceMap.sRegsSysPBase);
	gsSGXDeviceMap.ui32RegsSize = SYS_IONE_SGX_REGS_SIZE;

	gsSGXDeviceMap.ui32IRQ = SYS_IONE_SGX_IRQ;
...
PVRSRV_ERROR SysInitialise(IMG_VOID)
{
	...
	TimerRegPhysBase.uiAddr = SYS_IONE_GP11TIMER_REGS_SYS_PHYS_BASE;
...
PVRSRV_ERROR SysDeinitialise (SYS_DATA *psSysData)
{
	...
	if(SYS_SPECIFIC_DATA_TEST(gpsSysSpecificData, SYS_SPECIFIC_DATA_ENABLE_LOCATEDEV))
	{
		...
		OSBaseFreeContigMemory(SYS_IONE_SGX_REGS_SIZE, gsSGXRegsCPUVAddr, gsSGXDeviceMap.sRegsCpuPBase);
	}
#endif
