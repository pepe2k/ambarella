...
#if defined(SYS_IONE4_HAS_DVFS_FRAMEWORK)
#include <linux/opp.h>
#endif
...
#if defined(SUPPORT_IONE3430_SGXFCLK_96M)
...
#if (defined(DEBUG) || defined(TIMING)) && !defined(PVR_NO_IONE_TIMER)
#if defined(PVR_IONE_USE_DM_TIMER_API)
...
static PVRSRV_ERROR AcquireGPTimer(SYS_SPECIFIC_DATA *psSysSpecData)
{
	...
	psSysSpecData->psGPTimer = ione_dm_timer_request_specific(GPTIMER_TO_USE);
	if (psSysSpecData->psGPTimer == NULL)
	{
	
		PVR_DPF((PVR_DBG_WARNING, "%s: ione_dm_timer_request_specific failed", __FUNCTION__));
		return PVRSRV_ERROR_CLOCK_REQUEST_FAILED;
	}

	
	ione_dm_timer_set_source(psSysSpecData->psGPTimer, IONE_TIMER_SRC_SYS_CLK);
	ione_dm_timer_enable(psSysSpecData->psGPTimer);

	
	ione_dm_timer_set_load_start(psSysSpecData->psGPTimer, 1, 0);

	ione_dm_timer_start(psSysSpecData->psGPTimer);

	
	psSysSpecData->sTimerRegPhysBase.uiAddr = SYS_IONE4430_GP11TIMER_REGS_SYS_PHYS_BASE;
...
static void ReleaseGPTimer(SYS_SPECIFIC_DATA *psSysSpecData)
{
	if (psSysSpecData->psGPTimer != NULL)
	{
		...
		(void) ione_dm_timer_stop(psSysSpecData->psGPTimer);

		ione_dm_timer_disable(psSysSpecData->psGPTimer);

		ione_dm_timer_free(psSysSpecData->psGPTimer);
...
static PVRSRV_ERROR AcquireGPTimer(SYS_SPECIFIC_DATA *psSysSpecData)
{
#if defined(PVR_IONE4_TIMING_PRCM)
	struct clk *psCLK;
	IMG_INT res;
	struct clk *sys_ck;
	IMG_INT rate;
	...
	sTimerRegPhysBase.uiAddr = SYS_IONE4430_GP11TIMER_TSICR_SYS_PHYS_BASE;
	...
	sTimerRegPhysBase.uiAddr = SYS_IONE4430_GP11TIMER_ENABLE_SYS_PHYS_BASE;
	pui32TimerEnable = OSMapPhysToLin(sTimerRegPhysBase,
                  4,
                  PVRSRV_HAP_KERNEL_ONLY|PVRSRV_HAP_UNCACHED,
                  &hTimerEnable);
...
#if defined(SUPPORT_DRI_DRM_PLUGIN)
static struct ione_gpu_plugin sIONEGPUPlugin;
...
SysDRMRegisterPlugin(PVRSRV_DRM_PLUGIN *psDRMPlugin)
{
	...
	SYS_DRM_SET_PLUGIN_FIELD(&sIONEGPUPlugin, psDRMPlugin, name);
	SYS_DRM_SET_PLUGIN_FIELD(&sIONEGPUPlugin, psDRMPlugin, open);
	SYS_DRM_SET_PLUGIN_FIELD(&sIONEGPUPlugin, psDRMPlugin, load);
	SYS_DRM_SET_PLUGIN_FIELD(&sIONEGPUPlugin, psDRMPlugin, unload);
	SYS_DRM_SET_PLUGIN_FIELD(&sIONEGPUPlugin, psDRMPlugin, release);
	SYS_DRM_SET_PLUGIN_FIELD(&sIONEGPUPlugin, psDRMPlugin, mmap);
	SYS_DRM_SET_PLUGIN_FIELD(&sIONEGPUPlugin, psDRMPlugin, ioctls);
	SYS_DRM_SET_PLUGIN_FIELD(&sIONEGPUPlugin, psDRMPlugin, num_ioctls);
	SYS_DRM_SET_PLUGIN_FIELD(&sIONEGPUPlugin, psDRMPlugin, ioctl_start);

	iRes = ione_gpu_register_plugin(&sIONEGPUPlugin);
	if (iRes != 0)
	{
		PVR_DPF((PVR_DBG_ERROR, "%s: ione_gpu_register_plugin failed (%d)", __FUNCTION__, iRes));
	}
...
{
	int iRes = ione_gpu_unregister_plugin(&sIONEGPUPlugin);
	if (iRes != 0)
	{
		PVR_DPF((PVR_DBG_ERROR, "%s: ione_gpu_unregister_plugin failed (%d)", __FUNCTION__, iRes));
	}
}
#endif
