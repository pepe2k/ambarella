                   ...
                   IMG_PVOID pvPrivData, IMG_UINT32 ui32PrivDataLength)
{
    ...
    BUG_ON(ui32PrivDataLength != offsetof(struct ione_ion_tiler_alloc_data, handle));
    memcpy(&sAllocData, pvPrivData, offsetof(struct ione_ion_tiler_alloc_data, handle));

    if(ione_ion_tiler_alloc(gpsIONClient, &sAllocData) < 0)
    {
        PVR_DPF((PVR_DBG_ERROR, "%s: Failed to allocate via ion_tiler", __func__));
        goto err_free;
    }

    if(ione_tiler_pages(gpsIONClient, sAllocData.handle, &iNumPages,
						&pu32PageAddrs) < 0)
    {
