...
void IONELFBSwapHandler(IONELFB_BUFFER *psBuffer)
{
	...
	if (psSwapChain->pvsync_buffers[psSwapChain->enqueue_index].state == IONELFB_BUFFER_PENDING) {
		IONELFBWaitForVSync(psDevInfo);
	}

	memcpy(&psSwapChain->pvsync_buffers[psSwapChain->enqueue_index].buffer,	psBuffer, sizeof(*psBuffer));
	psSwapChain->pvsync_buffers[psSwapChain->enqueue_index++].state = IONELFB_BUFFER_PENDING;
	if (psSwapChain->enqueue_index >= psSwapChain->ulBufferCount) {
...
static IONELFB_ERROR IONELFBInitFBDev(IONELFB_DEVINFO *psDevInfo)
{
	...
	if (psLINFBInfo->var.xres > psLINFBInfo->var.yres) {
