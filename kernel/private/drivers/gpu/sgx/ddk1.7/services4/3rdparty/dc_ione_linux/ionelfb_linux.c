...
#ifndef CONFIG_SGX_QUEUE_EACH_FRAME
	unsigned long flags;
...
#else
	queue_work(psSwapChain->refresh_workqueue, &psSwapChain->refresh_worker);
#endif
...
void IONELFBRefreshWorker(struct work_struct *work)
{
	IONELFB_SWAPCHAIN	*psSwapChain;
	unsigned long		flags;
	unsigned int		wake_up = 0;

	psSwapChain = container_of(work, IONELFB_SWAPCHAIN, refresh_worker);

	if (psSwapChain->pvsync_buffers[psSwapChain->dequeue_index].state == IONELFB_BUFFER_PENDING) {
		IONELFB_BUFFER		*psBuffer = &psSwapChain->pvsync_buffers[psSwapChain->dequeue_index].buffer;
		IONELFBFlip(psBuffer->psDevInfo, psBuffer);
		psSwapChain->pvsync_buffers[psSwapChain->dequeue_index++].state = IONELFB_BUFFER_VSYNCED;
		if (psSwapChain->dequeue_index >= psSwapChain->ulBufferCount) {
			psSwapChain->dequeue_index = 0;
		}
	}
...
#ifdef CONFIG_SGX_QUEUE_EACH_FRAME
	sprintf(buf, "sgx_display%d", psSwapChain->uiFBDevID);
	psSwapChain->refresh_workqueue = create_singlethread_workqueue(buf);
	INIT_WORK(&psSwapChain->refresh_worker, IONELFBRefreshWorker);
	psSwapChain->pvsync_buffers = IONELFBAllocKernelMem(sizeof(IONELFB_VSYNC_BUFFER) * psSwapChain->ulBufferCount);
	if (!psSwapChain->pvsync_buffers) {
		printk(KERN_WARNING DRIVER_PREFIX "Alloc memory for vsync buffers Failed!\n");
		return (IONELFB_ERROR_INIT_FAILURE);
	}
	for (i = 0; i < psSwapChain->ulBufferCount; i++) {
		psSwapChain->pvsync_buffers[i].state = IONELFB_BUFFER_IDLE;
	}
	psSwapChain->enqueue_index = 0;
	psSwapChain->dequeue_index = 0;
#endif
...
#ifdef CONFIG_SGX_QUEUE_EACH_FRAME
	destroy_workqueue(psSwapChain->refresh_workqueue);
	if (psSwapChain->pvsync_buffers) {
		IONELFBFreeKernelMem(psSwapChain->pvsync_buffers);
	}
#endif
}
