...
#include "hw.h"
#ifdef CONFIG_ATH9K_EEPROM_FILE
...
#define CALADATA_FILE   "/lib/firmware/ath9k/ath9k_caldata.bin"

int ath9k_hw_get_eeprom_file(char *file_name,u16 *eep_data,int size)
{
	mm_segment_t	oldfs;
	struct file	*filp;
	struct inode	*inode = NULL;
	int			length;
	unsigned char	eeptmp[ATH9K_CALADATA_SIZE];
	u16	*data;
	int	i;
	...
	oldfs = get_fs();
	set_fs(KERNEL_DS);
	filp = filp_open(file_name, O_RDONLY, S_IRUSR);
	...
	/* read data */
	if (filp->f_op->read(filp, eeptmp, ATH9K_CALADATA_SIZE, &filp->f_pos) != ATH9K_CALADATA_SIZE) {
...
{
#ifdef CONFIG_ATH9K_EEPROM_FILE
	if(ath9k_hw_get_eeprom_file(CALADATA_FILE,eep_data,size)<0){
		printk("Get Caldata file error\n");
	}
#else
