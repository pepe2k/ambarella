...
static void __init ambarella_init_coconut(void)
{
	...
	if (AMBARELLA_BOARD_TYPE(system_rev) == AMBARELLA_BOARD_TYPE_EVK) {
		...
		if (AMBARELLA_BOARD_REV(system_rev) == 104) {
			...
			printk("wifi_power init coconut 1111\n");
			ambarella_board_generic.wifi_power.gpio_id = GPIO(12);
			ambarella_board_generic.wifi_power.active_level = GPIO_HIGH;
			ambarella_board_generic.wifi_power.active_delay = 300;
			ambarella_board_generic.wifi_sd_bus = 0;
			ambarella_board_generic.wifi_sd_slot = 1;

			ambarella_platform_sd_controller0.slot[1].ext_power.gpio_id = GPIO(30);
...
#endif
#if defined(CONFIG_CODEC_AMBARELLA_AK4954)
		ambarella_init_ak4954(0, 0x12, GPIO(41));
#endif
	}

#if defined(CONFIG_CODEC_AMBARELLA_FM34_500B)
	ambarella_init_fm34(0, GPIO(12), 20, GPIO(83), GPIO(82));
#endif
	/* Register devices */
