...
static void ginkgo_ipcam_sd_set_bus_timing(u32 timing)
{
	...
	if (timing == MMC_TIMING_UHS_DDR50) {
		ms_delay_reg |= ((0x2 << 26) | (0x7 << 16) | (0x7 << 10));
	}
...
static void ginkgo_ipcam_sdio_set_bus_timing(u32 timing)
{
	...
	if (timing == MMC_TIMING_UHS_DDR50) {
		ms_delay_reg |= ((0x2 << 29) | (0x7 << 21) | (0x7 << 13));
	}
	amba_rct_writel(MS_DELAY_CTRL_REG, ms_delay_reg);
}
	...
	if (AMBARELLA_BOARD_TYPE(system_rev) == AMBARELLA_BOARD_TYPE_IPCAM) {
		...
		case 'A':
			...
			case 224000000:	//S233
				...
				break;
			case 264000000:	//S255
				ambarella_platform_sd_controller0.max_clock = 49500000;
				ambarella_platform_sd_controller1.max_clock = 49500000;
				break;
			case 384000000:	//S266, S288
			...
			ambarella_board_generic.vin0_reset.gpio_id =  EXT_GPIO(8);
