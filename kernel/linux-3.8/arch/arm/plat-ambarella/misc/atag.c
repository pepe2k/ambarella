...
#if defined(CONFIG_PLAT_AMBARELLA_S2)
		//We do not MAP Whole DSP memory On S2
		if (i == AMBARELLA_IO_DESC_DSP_ID) {
			continue;
		}
#endif
...
/* ==========================================================================*/
#if defined(CONFIG_PLAT_AMBARELLA_S2)
// IAV will take control of DSP memory right now. Kernel will not touch it.
static int __init dsp_mem_check(u32 start, u32 size)
{
	ambarella_io_desc[AMBARELLA_IO_DESC_DSP_ID].io_desc.virtual = 0;
...
void set_ambarella_dspmem_virt(u32 addr)
{
	ambarella_io_desc[AMBARELLA_IO_DESC_DSP_ID].io_desc.virtual = addr;
}
EXPORT_SYMBOL(set_ambarella_dspmem_virt);
