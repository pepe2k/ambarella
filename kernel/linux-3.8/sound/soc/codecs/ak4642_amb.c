...
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	unsigned short val, val1, val2, val3;

	val1 = snd_soc_read(codec, AK4642_PM3);
	val2 = snd_soc_read(codec, AK4642_SIG1);
	val3 = snd_soc_read(codec, AK4642_PM1);

	if(((val1 & 0x1e) == 0x06) && ((val2 & 0x04) == 0x00)){
		val = AK4642_LINE_IN_ON;
	}else if(((val1 & 0x19) == 0x18) && ((val2 & 0x04) == 0x04) &&
											((val3 & 0x1) == 0x1)){
		val = AK4642_LEFT_MIC_MONO_ON;
	}else if(((val1 & 0x19) == 0x19) && ((val2 & 0x04) == 0x04) &&
											((val3 & 0x1) == 0x0)){
		val = AK4642_RIGHT_MIC_MONO_ON;
	}else if(((val1 & 0x19) == 0x19) && ((val2 & 0x04) == 0x04) &&
											((val3 & 0x1) == 0x1)){
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	case AK4642_BOTH_MIC_ON:
		snd_soc_update_bits(codec, AK4642_PM3, 0x19, 0x19);//right on
		snd_soc_update_bits(codec, AK4642_SIG1, 0x05, 0x05);
		snd_soc_update_bits(codec, AK4642_SIG2, 0x20, 0x20);
		snd_soc_update_bits(codec, AK4642_PM1, 0x01,0x01);//left on
		break;
	case AK4642_LEFT_MIC_MONO_ON:
		snd_soc_update_bits(codec, AK4642_PM3, 0x19, 0x18);// right mute
		snd_soc_update_bits(codec, AK4642_SIG1, 0x05, 0x05);
		snd_soc_update_bits(codec, AK4642_SIG2, 0x20, 0x20);
		snd_soc_update_bits(codec, AK4642_PM1, 0x01,0x01);// left on
		break;
	case AK4642_RIGHT_MIC_MONO_ON:
		snd_soc_update_bits(codec, AK4642_PM3, 0x19, 0x19);// right on
		snd_soc_update_bits(codec, AK4642_SIG1, 0x05, 0x05);
		snd_soc_update_bits(codec, AK4642_SIG2, 0x20, 0x20);
		snd_soc_update_bits(codec, AK4642_PM1, 0x01,0x00);// left mute
		break;
...
static const char *ak4642_input_mux[] = {"Line-in", "Both Mic", "Left Mic Mono","Right Mic Mono","Unknown"};
...
static const struct soc_enum ak4642_enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(5, ak4642_input_mux),
};
			    ...
			    struct snd_soc_dai *dai)
{
	...
	struct ak4642_priv *ak4642 = snd_soc_codec_get_drvdata(codec);
	int rate = params_rate(params), fs = 256, pll = 0;
	u8 mode = snd_soc_read(codec, AK4642_MODE2) & 0xc0;
	u8 mode1 = snd_soc_read(codec, AK4642_MODE1);
	int i = 0;
	if (ak4642->clkid == AK4642_MCLK_IN){
		if (rate)
		...
		/* set rate */
		snd_soc_write(codec, AK4642_MODE1, mode1 | 0x40);
		snd_soc_write(codec, AK4642_MODE2, mode);
		snd_soc_update_bits(codec,AK4642_PM2,0x09,0x00);// slave and pll off
	}else if (ak4642->clkid == AK4642_BCLK_IN) {
		if (ak4642->sysclk <= 8000) {
			mode |= 0x00;
		}else if (ak4642->sysclk > 8000 && ak4642->sysclk <= 12000) {
			mode |= 0x01;
		}else if (ak4642->sysclk > 12000 && ak4642->sysclk <= 16000) {
			mode |= 0x02;
		}else if (ak4642->sysclk > 16000 && ak4642->sysclk <= 24000) {
			mode |= 0x03;
		}else if (ak4642->sysclk > 24000 && ak4642->sysclk <= 32000) {
			mode |= 0x0a;
		}else if (ak4642->sysclk > 32000 && ak4642->sysclk <= 48000) {
			mode |= 0xb;
		}
		snd_soc_write(codec, AK4642_MODE1, mode1 & 0xbf);
		snd_soc_write(codec, AK4642_MODE1, mode1 | 0x20);
		snd_soc_write(codec, AK4642_MODE2, mode);
		snd_soc_update_bits(codec,AK4642_PM2,0x09,0x01);// slave and pll on
	} else if (ak4642->clkid == AK4642_MCLK_IN_BCLK_OUT) {
		if (ak4642->sysclk == 11289600) {
			pll = 0x4;
		} else if (ak4642->sysclk == 12288000) {
			pll = 0x5;
		} else if (ak4642->sysclk == 12000000) {
			pll = 0x6;
		} else if (ak4642->sysclk == 24000000) {
			pll = 0x7;
		} else if (ak4642->sysclk == 13500000) {
			pll = 0xc;
		} else if (ak4642->sysclk == 27000000) {
			pll = 0xd;
		}
		for (i=0; rate_table[i][0] != 0; i++) {
		...
		fs = ((fs&0x8)<<2)|(fs&0x7);
		snd_soc_update_bits(codec, AK4642_MODE1, 0xf0, pll<<4);
		snd_soc_update_bits(codec, AK4642_MODE2, 0x27, fs);
		snd_soc_update_bits(codec, AK4642_PM2,0x09,0x09);//master and pll on
	}
	return 0;
}
