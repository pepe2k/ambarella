...
{
	struct ak4954_priv *ak4954 = snd_soc_codec_get_drvdata(codec);
	struct ak4954_platform_data *ak4954_pdata;
	int ret = 0;

	akdbgprt("\t[AK4954] %s(%d)\n",__FUNCTION__,__LINE__);

	ak4954_pdata = codec->dev->platform_data;
	if(!ak4954_pdata)
		return -EINVAL;
...
#if 0
	ret = devm_gpio_request(codec->dev, ak4954->rst_pin, "ak4954 reset");
	if (ret < 0){
	...
	if (gpio_is_valid(ak4954_pdata->rst_pin)) {
		ret = gpio_request(ak4954_pdata->rst_pin, "ak4954 reset");
		if (ret < 0)
			return ret;
	} else {
		return -ENODEV;
	...
	/* Reset AK4954 codec */
	gpio_direction_output(ak4954_pdata->rst_pin, GPIO_LOW);
	msleep(ak4954_pdata->rst_delay);
	gpio_direction_output(ak4954_pdata->rst_pin, GPIO_HIGH);
...
static int ak4954_remove(struct snd_soc_codec *codec)
{
	...
	ak4954_pdata = codec->dev->platform_data;
	gpio_free(ak4954_pdata->rst_pin);
