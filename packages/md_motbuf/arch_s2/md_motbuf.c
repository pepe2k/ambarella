...
static int unmap_buffer(void)
{
	...
	if (ioctl(fd_iav, IAV_IOC_UNMAP_BSB, &info) < 0) {
	...
	if (ioctl(fd_iav, IAV_IOC_UNMAP_DSP, &info) < 0) {
...
static u32 algo_framediff_default(md_motbuf_roi_t *roi, u8* cur_data, u8* pre_data)
{
	...
	for (i=0; i<roi_height; i++) {
		for (j=0; j<roi_width; j++) {
			index = buf_pitch * (roi->roi_location.y + i*md_me_buffer_init.pixel_line_interval) + roi->roi_location.x + j;
...
static void *md_motbuf_mainloop(void *arg)
{
	...
	u32	i, height, pitch;
	u8 *addr, *cur_data = NULL, *pre_data = NULL;
	int md_evt_flag = 0;
	...
	info.source = md_me_buffer_init.me_buffer;
	...
	pitch = info.pitch;
	height =info.height;
	memcpy(me_y_buffer[cur_buf_id], addr, pitch * height);
...
int md_motbuf_yuv2graybmp(md_motbuf_roi_t roi, char *bmpname, int width, int height)
{
	...
	do {
		...
		for (i = 0; i < height; i ++) {
			...
			for (j = 0; j < width; j += 2) {
				index = buf_pitch * (roi.roi_location.y + i)
...
int md_motbuf_get_current_buffer_P_W_H(int *buffer_pitch, int *buffer_width, int *buffer_height)
{
	...
	buf_pitch = *buffer_pitch = info.pitch;
	buf_width = *buffer_width = info.width;
	buf_height = *buffer_height = info.height;
