...
int enable_cc(int fd_iav, char* sensor_name, AMBA_DSP_IMG_MODE_CFG_s* ik_mode)
{
	...
	AmbaDSP_ImgSetColorCorrection(fd_iav, ik_mode,&cc_3d);

	return 0;
}

int load_containers(u32 sensor_id,char* sensor_name)
{
	fc_collection_t fcc;
...
int img_aaa_config(int fd_iav,u32 sensor_id)
{
	int i =0;
	ae_cfg_tbl_t ae_tbl[4];
	amba_vin_agc_info_t agc_info;
	ioctl(fd_iav, IAV_IOC_VIN_SRC_GET_AGC_INFO, &agc_info);
	switch(sensor_id){
		case SENSOR_MN34210PL:
			...
			for(i=0; i<4; i++) {
				...
				ae_tbl[i].line_num = 4;
				ae_tbl[i].belt = 4;
				ae_tbl[i].db_step = agc_info.db_step;
