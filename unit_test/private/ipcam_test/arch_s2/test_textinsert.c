 ...
 *
 * History:
 *  2012/02/25 - [Jian Tang] created file
 * Copyrigt (C) 2012-2016, Ambarella, Inc
 *
...
static struct option long_options[] = {
	...
	{ "osd-insert-always", HAS_ARG, 0, 'i'},
	{ 0, 0, 0, 0 }
};

static const char *short_options = "ABCDa:x:y:w:h:i:t:s:c:";
...
static int init_param(int argc, char **argv)
{
	...
	while ((ch = getopt_long(argc, argv, short_options, long_options, &option_index))
		...
		case 'i':
			...
			overlay_insert[stream_id].insert_always = osd_insert_always;
			break;
	...
	for (i = 0; i < MAX_ENCODE_STREAM_NUM; i++) {
		if (stream_text_info[i].enable) {
			...
			if (stream_text_info[i].rotate == AUTO_ROTATE) {
				rotate_clockwise = format.rotate_clockwise;
				hflip = format.hflip;
				vflip = format.vflip;
		...
		for (j = 0; j < MAX_OVERLAY_AREA_NUM; j++) {
			...
			if (area->enable && osd_time->enable) {
				...
				while (current_time[k] != '\0' &&
						...
						osd_time->osd_y[k] != NOT_CONVERTED) {
