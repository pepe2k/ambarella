...
typedef struct qp_roi_s {
	int quality_level[QP_FRAME_TYPE_NUM];	// 0 means the ROI is in the default QP RC.
	int start_x;
	int start_y;
	int width;
	int height;
...
static int show_stream_menu(int stream_id)
{
	...
	if (stream_roi[stream_id].qp_type) {
		...
		printf("| 7 - Test ROI map for frame I/P/B  \t|\n");
		printf("| 8 - Set ROI map of frame I/P/B for test \t|\n");
...
static int get_qp_roi(int stream_id, iav_qp_roi_matrix_ex_t *matrix)
{
	...
	matrix->daddr[QP_FRAME_I] = (u8 *) (qp_matrix_addr + stream_qp_matrix_size * stream_id);
...
static int set_qp_roi(int stream_id)
{
	...
	if ((buf_pitch * buf_height) > (stream_qp_matrix_size / QP_FRAME_TYPE_NUM)) {
		printf("Error: The size of QP ROI matrix is not enough\n");
		return -1;
	}
	...
	if (qp_matrix.type) {
		for (k = QP_FRAME_I; k < QP_FRAME_TYPE_NUM; k++) {
			addr = (iav_matrix_data_t *)(qp_matrix_addr +
				stream_qp_matrix_size * stream_id + stream_qp_matrix_size * k /
				QP_FRAME_TYPE_NUM);
			for (i = start_y; i < end_y && i < buf_height; i++) {
				for (j = start_x; j < end_x && j < buf_width; j++) {
					/* bit8~15 is used for advance qp roi type */
					addr[i * buf_pitch + j].adv_qp_data =
						qp_roi[stream_id].quality_level[k] +
						QP_MATRIX_ADV_DEFAULT_VALUE;
				}
			}
			qp_matrix.daddr[k] = (u8 *) addr;
		}
	} else {
		addr = (iav_matrix_data_t *)(qp_matrix_addr +
			stream_qp_matrix_size * stream_id);
		for (i = start_y; i < end_y && i < buf_height; i++) {
			for (j = start_x; j < end_x && j < buf_width; j++)
				addr[i * buf_pitch + j].basic_qp_data =
					qp_roi[stream_id].quality_level[QP_FRAME_I];
		}
	}
...
static int clear_qp_roi(int stream_id)
{
	iav_qp_roi_matrix_ex_t qp_matrix;
	int i;
	iav_matrix_data_t *addr =
		(iav_matrix_data_t *)(qp_matrix_addr + stream_qp_matrix_size * stream_id);
	...
	if (qp_matrix.type) {
		for (i = 0; i < clear_size; i++) {
			addr[i].adv_qp_data = QP_MATRIX_ADV_DEFAULT_VALUE;
		}
	} else {
	...
	if (qp_matrix.enable) {
		if (qp_matrix.type) {
			for (k = QP_FRAME_I; k < QP_FRAME_TYPE_NUM; k++) {
				addr = (iav_matrix_data_t *)(qp_matrix_addr +
					stream_qp_matrix_size * stream_id +
					stream_qp_matrix_size * k / QP_FRAME_TYPE_NUM);
				printf("\n\n===============================================\n");
				switch (k) {
					case QP_FRAME_I:
						printf("Qisplay qp roi advance map for Frame I\n");
						break;
					case QP_FRAME_P:
						printf("Qisplay qp roi advance map for Frame P\n");
						break;
					case QP_FRAME_B:
						printf("Qisplay qp roi advance map for Frame B\n");
						break;
					default:
						printf("Error:No this frame type\n");
						return -1;
				}
				printf("\n\n===============================================\n");
				for (i = 0; i < buf_height; i++) {
					...
					for (j = 0; j < buf_width; j++)
						/* bit8~15 is used for advance qp roi type */
						printf("%4d", addr[i * buf_pitch + j].adv_qp_data -
						QP_MATRIX_ADV_DEFAULT_VALUE);
				}
			}
		} else {
			...
			for (i = 0; i < buf_height; i++) {
				printf("\n");
				for (j = 0; j < buf_width; j++)
					printf("%-2d", addr[i * buf_pitch + j].basic_qp_data);
			}
			printf("\n");
		}
	}
	...
	for (k = QP_FRAME_I; k < QP_FRAME_TYPE_NUM; k++) {
		...
		for (i = start_y; i < end_y; i++) {
			for (j = start_x; j < end_x; j++) {
				...
				/* bit8~15 is used for advance qp roi type */
				addr[i * buf_pitch + j].adv_qp_data = data;
			}
		}
		qp_matrix.daddr[k] = (u8 *) addr;
...
static int adv_qp_roi_table_test(int stream_id)
{
	iav_qp_roi_matrix_ex_t qp_matrix;
	int i, j, k;
	iav_matrix_data_t *addr = NULL;
	...
	for (k = QP_FRAME_I; k < QP_FRAME_TYPE_NUM; k++) {
		...
		switch (k) {
			case QP_FRAME_I:
				data = QP_MATRIX_ADV_DEFAULT_VALUE +
					qp_roi[stream_id].quality_level[QP_FRAME_I];
				break;
			case QP_FRAME_P:
				data = QP_MATRIX_ADV_DEFAULT_VALUE +
					qp_roi[stream_id].quality_level[QP_FRAME_P];
				break;
			case QP_FRAME_B:
				data = QP_MATRIX_ADV_DEFAULT_VALUE +
					qp_roi[stream_id].quality_level[QP_FRAME_B];
				break;
			default:
				printf("Error: No this frame type.\n");
				return -1;
...
static int config_stream_roi(int stream_id)
{
	...
	while (back2main == 0) {
			...
			if (opt == '2') {
				if (stream_roi[stream_id].qp_type) {
					for (k = QP_FRAME_I; k < QP_FRAME_TYPE_NUM; k++) {
						switch (k) {
							case QP_FRAME_I:
								printf("Input ROI quality level (-51~51) for I frame: ");
								break;
							case QP_FRAME_P:
								printf("Input ROI quality level (-51~51) for P frame: ");
								break;
							case QP_FRAME_B:
								printf("Input ROI quality level (-51~51) for B frame: ");
								break;
							default:
								printf("Error: No this frame type\n");
								return -1;
						}
						qp_roi[stream_id].quality_level[k] = input_value(-51, 51);
					}
				} else {
					printf("Input ROI quality level (1~%d): ", MAX_QP_DELTA_NUM);
					qp_roi[stream_id].quality_level[QP_FRAME_I] =
						input_value(1, MAX_QP_DELTA_NUM);
				}
			} else {
				for (k = QP_FRAME_I; k < QP_FRAME_TYPE_NUM; k++) {
					qp_roi[stream_id].quality_level[k] = 0;
				}
			}
		...
		case '7':
			if (!stream_roi[stream_id].qp_type) {
				printf("List roi I/P/B offset tables for advance type\n");
				break;
			}
			if (adv_qp_roi_table_test(stream_id) < 0) {
				return -1;
			}
			break;
		case '8':
