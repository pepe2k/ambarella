...
//        ret = strerror_r(AVUNERROR(errnum), errbuf, errbuf_size);
        char*  err_msg = strerror_r(AVUNERROR(errnum), errbuf, errbuf_size);//The GNU-specific strerror_r() returns a pointer to a string containing the error message
        av_log(NULL, AV_LOG_DEBUG, "err_msg=%s\n", err_msg);
#else
        ret = -1;
#endif
        if (ret < 0)
