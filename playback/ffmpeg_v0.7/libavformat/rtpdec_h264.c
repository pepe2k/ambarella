                               ...
                               PayloadContext *h264_data, const char *line)
{
    ...
    if (av_strstart(p, "framesize:", &p)) {
    ...
    }else if(av_strstart(p, "framerate:", &p)){
        //take for framrate:29.970000 case
        char buf[32];
        char* dst = buf;
        const char* tmp;
        int integer, fraction, len = 0, num = 1;

        while (*p && *p == ' ') p++; // strip spaces.
        while (*p && *p != '.' && (dst - buf) < sizeof(buf) - 1) {
            *dst++ = *p++;
        }
        tmp = p;
        ...
        *dst = '\0';
        integer = atoi(buf);
        fraction = atoi(tmp + 1); // skip the .
        av_reduce(&(stream->avg_frame_rate.num), &(stream->avg_frame_rate.den),
                      integer*num, (fraction==0)?num:fraction, INT_MAX);
        av_reduce(&(stream->r_frame_rate.num), &(stream->r_frame_rate.den),
                      integer*num, (fraction==0)?num:fraction, INT_MAX);
        av_log(NULL, AV_LOG_INFO, "dump frame_rate num:%d, dem:%d, inter:%d, fraction:%d, num:%d\n", stream->avg_frame_rate.num, stream->avg_frame_rate.den
