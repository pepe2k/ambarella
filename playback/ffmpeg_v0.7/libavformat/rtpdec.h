...
#if PROJECT_NVR
    /*Fields for h264 packet-loss processing, @{
    *        When 'handle_h264_packet_loss' set 1, the whole frame will be discarded when packet-loss occurs, and
    *   'handle_h264_combine_frame' is effective.
    * Now "packet-loss processing" uses the  reording queue, TODO
    */
    int handle_h264_packet_loss;
    int handle_h264_combine_frame;
    RTPPacket* recv_queue;
    int recv_queue_len;
    uint32_t recv_queue_timestamp;
    uint16_t h264_parsed_seq;
