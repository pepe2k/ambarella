                            ...
                            AVFormatParameters *ap)
{
    ...
    if (ap->initial_pause) {
        ...
        /*MPEGTS over RTP, continue parsing until PAT/PMT parsed*/
        if(rt->nb_rtsp_streams >0){
            RTSPStream *rtsp_st = rt->rtsp_streams[rt->nb_rtsp_streams - 1];
            if (!strcmp(ff_rtp_enc_name(rtsp_st->sdp_payload_type), "MP2T")){
                s->is_mpegts_over_rtp = 1;
                s->mpegts_rtp_info_parsed = 0;
                int count  = 0;
                do{
                    AVPacket pkt;
                    av_init_packet(&pkt);
                    int ret = rtsp_read_packet(s, &pkt);
                    if(ret == AVERROR(EAGAIN)){
                        //printf("rtsp_read_packet EAGAIN\n");
                        continue;
                    }
                    if(ret < 0){
                        break;
                ...
                }while(!s->mpegts_rtp_info_parsed);
                if(!s->mpegts_rtp_info_parsed){
                    printf("MP2T --- PAT/PMT not parsed, ERROR\n");
                }
            }
        }
    }
...
int ff_rtsp_tcp_read_packet(AVFormatContext *s, RTSPStream **prtsp_st,
                            uint8_t **buf_ptr, int *buf_size_ptr)
{
    RTSPState *rt = s->priv_data;
    int id, len, i, ret;
    RTSPStream *rtsp_st;
...
redo:
    ...
    /*realloc memroy to process very large Unit*/
    if(len > buf_size){
        int buf_size_new = (len + 15)/16*16;
        uint8_t  *buf_new = (uint8_t*)av_malloc(buf_size_new);
        if(!buf_new){
            goto redo;
        }
        buf = buf_new;
    ...
    if (ret != len){
        if(buf != (*buf_ptr)){
            av_free(buf);//free the reallocated buffer
        }
        return -1;
    }
...
found:
    if(buf != (*buf_ptr)){
        //buf reallocated, free the old and use the new one
        av_free(*buf_ptr);
        *buf_ptr = buf;
...
#endif
static void  rtsp_resetup_free_context_streams(AVFormatContext *s)
{
    AVStream *st;
    int i;
...
retry:
    ...
    if (ret < 0) {
#if PROJECT_NVR
        if (ret == AVERROR(EAGAIN) && rt->transport == RTSP_TRANSPORT_RTP) {
            goto nvr_next;
        }
#endif
        if (ret == AVERROR(ETIMEDOUT) && !rt->packets) {
            if (rt->lower_transport == RTSP_LOWER_TRANSPORT_UDP &&
...
#else
                {
                    ...
                    rtsp_resetup_free_context_streams(s);
                    memset(rt,0,sizeof(*rt));

                    if(!retry_count){
                        int len = strlen(s->filename);
                        if(len + 5 > sizeof(s->filename)){
                            return -1;
                        }
                        s->filename[len]= '?';
                        s->filename[len + 1]= 't';
...
nvr_next:
    ...
    rtsp_lock(rt);
    last_cmd_time = rt->last_cmd_time;
    rtsp_unlock(rt);
    if ((av_gettime() - last_cmd_time) / 1000000 >= rt->timeout / 2) {
       ...
       {
            size_t total = ff_get_mem_allocated_size();
            av_log(NULL, AV_LOG_WARNING, "NVR_DEBUG (rtsp %p), total mem allocated size is %8u\n",rt,total);
       }
#endif
    }
