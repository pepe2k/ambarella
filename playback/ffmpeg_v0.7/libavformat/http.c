...
typedef struct {
    ...
    int support_range;    /**fix bug 1323: some video website(e.g youku) don't support "Range" in request header. */
} HTTPContext;
...
static int http_open_cnx(URLContext *h)
{
 ...
 redo:
    ...
    if (http_connect(h, path, hoststr, auth, &location_changed) < 0){
        if(s->http_code == 403 && s->support_range == 1){
            //if http_code is 403, set support_range to 0 and try again
            s->support_range = 0;
            goto redo;
        }else{
            goto fail;
                        ...
                        const char *auth, int *new_location)
{
    ...
    /*  fix bug1349: support sohu HD video client
     *  cause:       sohu server does filter process for android client player
     *  solution:    simulate well-done player, who can interact normally with server.
     *               adjust order of property in HTTP header
     *  changer:     jywang@ambarella.com
     */
...
static int http_read(URLContext *h, const uint8_t *buf, int size)
{
    HTTPContext *s = h->priv_data;
    int len;
    ...
    if (len > 0) {
        if (len > size)
            len = size;
        memcpy((unsigned char *)buf, s->buf_ptr, len);
        s->buf_ptr += len;
    } else {
        if (!s->willclose && s->filesize >= 0 && s->off >= s->filesize)
            return AVERROR_EOF;
        len = ffurl_read(s->hd, (unsigned char *)buf, size);
    }
    if (len > 0) {
        s->off += len;
        if (s->chunksize > 0)
