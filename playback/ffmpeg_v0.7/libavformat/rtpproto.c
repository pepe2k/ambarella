...
#if PROJECT_NVR
#define RTP_VIDEO_RX_BUF_SIZE  (1024 * 1024 * 4)
#endif
...
#if PROJECT_NVR
static void setUdpSockRecvBuf(int fd, int bufsize){
     int  tmp = bufsize;
     if (setsockopt(fd, SOL_SOCKET, SO_RCVBUFFORCE, &tmp, sizeof(tmp)) < 0) {
        av_log(NULL, AV_LOG_WARNING, "setsockopt(SO_RECVBUF): %s\n", strerror(errno));
        return;
     }

     {
         //for debug, show the changed RCVBUF size
         int recv_size = 0;
         socklen_t optlen = sizeof(recv_size);
         getsockopt(fd, SOL_SOCKET, SO_RCVBUFFORCE, (char*)&recv_size, &optlen);
         av_log(NULL, AV_LOG_ERROR, "setUdpSockRecvBuf -- size  = %d, %d\n", tmp,recv_size);
     }
}
#endif
...
static int rtp_open(URLContext *h, const char *uri, int flags)
{
    ...
    int rtp_port, rtcp_port,
        is_output, ttl, connect,codec_type,
    ...
    av_log(NULL, AV_LOG_DEBUG, "is_output=%d, codec_type=%d\n", is_output, codec_type);
    ...
    if (p) {
        ...
        if (av_find_info_tag(buf, sizeof(buf), "codec_type", p)) {
            codec_type = strtol(buf, NULL, 10);
        }
    }
...
#if PROJECT_NVR
     /*For NVR project,
     *      set rtp recv buffer size to the largest possible udp packet size to
     * avoid losing data on OSes that set this too low by default.
     *
     *  "net.core.rmem_max" etc. should be set properly, for example
    ...
    if(codec_type == AVMEDIA_TYPE_VIDEO){
        setUdpSockRecvBuf(s->rtp_fd,RTP_VIDEO_RX_BUF_SIZE);
    }else{
        setUdpSockRecvBuf(s->rtp_fd,RTP_RX_BUF_SIZE);
    }
#endif
...
static int rtp_read(URLContext *h, const uint8_t *buf, int size)
{
    RTPContext *s = h->priv_data;
    ...
    for(;;) {
        ...
        if (n > 0) {
            ...
            if (p[1].revents & POLLIN) {
                from_len = sizeof(from);
                len = recvfrom (s->rtcp_fd, (uint8_t *)buf, size, 0,
                                (struct sockaddr *)&from, &from_len);
                if (len < 0) {
                    if (ff_neterrno() == AVERROR(EAGAIN) ||
                        ff_neterrno() == AVERROR(EINTR))
            ...
            if (p[0].revents & POLLIN) {
                from_len = sizeof(from);
                len = recvfrom (s->rtp_fd, (uint8_t *)buf, size, 0,
                                (struct sockaddr *)&from, &from_len);
                if (len < 0) {
                    if (ff_neterrno() == AVERROR(EAGAIN) ||
                        ff_neterrno() == AVERROR(EINTR))
