...
static int rtmp_read(URLContext *s, const uint8_t *buf, int size)
{
    RTMPContext *rt = s->priv_data;
    int orig_size = size;
    int ret;
    ...
    while (size > 0) {
        ...
        if (data_left >= size) {
            memcpy((uint8_t *)buf, rt->flv_data + rt->flv_off, size);
            rt->flv_off += size;
            return orig_size;
        }
        if (data_left > 0) {
            memcpy((uint8_t *)buf, rt->flv_data + rt->flv_off, data_left);
            buf  += data_left;
            size -= data_left;
