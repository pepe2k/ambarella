...
#if 0
static uint8_t *TS_find_first_Pic_start_code(uint8_t *packet_start_pos, int pk_size)
{
    unsigned int state;
    int n = pk_size;
    uint8_t * pos_cur=packet_start_pos;

    if(NULL==packet_start_pos)
        return NULL;

    while (n > 0) {
        if (pos_cur+4>packet_start_pos+pk_size)
            break;
        state=((uint32_t *)pos_cur)[0];//little endine
        if (state == 0x00010000) {
#ifdef PES_HEADER_DEBUG_INFO
            av_log(NULL,AV_LOG_ERROR,"xxxx startcode=%x pos=0x%"PRIx64"\n", state, pos_cur);
#endif
            return pos_cur;
        }
        pos_cur++;
...
static const StreamType ISO_types[] = {
    ...
    { 0x91, AVMEDIA_TYPE_AUDIO,        CODEC_ID_PCM_MULAW },
    { 0 },
};
...
static void new_pes_packet(PESContext *pes, AVPacket *pkt)
{
    ...
    //av_log(NULL, AV_LOG_INFO, "PKT PTS:%lld\n", pkt->pts);
    /* reset pts values */
...
//added AMBA PES header in decoder instead of adding PES header in demuxer, 2012.01.05, roy mdf
//                    pes->header[4]=pes->header[5]=0;//make pes_header_len eaquls to 0, or will cause confusing segment fault, 2011.12.20, roy mdf
        ...
        case MPEGTS_PESHEADER_FILL:
            ...
            if (pes->data_index == pes->pes_header_size) {
                ...
                if ((flags & 0xc0) == 0x80) {
                    ...
                    r += 5;
                    if(pes->pts<pes->dts) {
                        av_log(pes->stream, AV_LOG_WARNING, "pts=%lld, dts=%lld, invalid should mdf.\n",pes->pts,pes->dts);//bug#750 Pioneer*.ts, 2011.12.21 roy mdf ver2
                        //pes->pts=pes->dts=AV_NOPTS_VALUE;
                        pes->dts=pes->pts;//mdf for MPEG12 PES header new format, 2012.02.10, roy
                        av_log(pes->stream, AV_LOG_WARNING, "pts=%lld, dts=%lld, mdfed.\n",pes->pts,pes->dts);//2011.12.23, add clue for this operation, roy mdf
                    }
                }
                ...
                /* Action:
                 * insert pes header at the beginning of es for dsp
                 *
                 * Cause:
                 * video decoder doesn't feed pes header wrapper into udec for MPEG1&MPEG2 video
                 * so demux should do it and couldn't remove pes header
                 *
                 * stream_type see ISO/IEC 13818-1 Table 2-36
                 * 0x01: ISO/IEC 11172 Video                        -- MPEG-1 video
                 * 0x02: ITU-T Rec. H.262 | ISO/IEC 13818-2 Video   -- MPEG-2 video
                 *
                 * Changed by:
                 * jywang on 2011-08-25

                if (pes->stream_type == 0x01 || pes->stream_type == 0x02)
                {
                    memcpy(pes->buffer, pes->header, pes->pes_header_size);
                    pes->data_index += pes->pes_header_size;
                }*///for MPEG12 PES header should ONLY be added before picture start code(0x00000100), 2011.12.19 roy mdf
            }
            break;
        case MPEGTS_PAYLOAD:
...
#if 0//added AMBA PES header in decoder instead of adding PES header in demuxer, 2012.01.05, roy mdf
                if ((pes->stream_type == 0x01 || pes->stream_type == 0x02) && (pes->pts != AV_NOPTS_VALUE)) //2011.12.19 roy mdf for MPEG12 TS PES header
                {
                    uint8_t *packet_start_pos = p;
                    uint8_t * startcode_base_pos = NULL;
                    int64_t bef_data_len=0;
                    startcode_base_pos = TS_find_first_Pic_start_code(packet_start_pos, buf_size);//now pos to start code BASE addr
#ifdef PES_HEADER_DEBUG_INFO
                    av_log(NULL,AV_LOG_ERROR,"TS_find_first_Pic_start_code(), startcode_base_pos-packet_start_pos=%lld.\n",startcode_base_pos-packet_start_pos);
#endif
                    if (NULL==startcode_base_pos || startcode_base_pos<packet_start_pos)
                    {
#ifdef PES_HEADER_DEBUG_INFO
                        av_log(NULL,AV_LOG_ERROR,"|| no find picture_start_code, go on.\n");
#endif
                    ...
                    {
                        bef_data_len = startcode_base_pos-packet_start_pos;
#ifdef PES_HEADER_DEBUG_INFO
                        av_log(NULL,AV_LOG_ERROR,"<--- buf_size=%d, bef_data_len=%lld, buf_size-bef_data_len=%d.\n",buf_size,bef_data_len,buf_size-bef_data_len);
#endif
                        if(buf_size>=bef_data_len)
                        {
                            memcpy(pes->buffer+pes->data_index, p, bef_data_len);
                            memcpy(pes->buffer+pes->data_index+bef_data_len, pes->header, pes->pes_header_size);
                            memcpy(pes->buffer+pes->data_index+bef_data_len+pes->pes_header_size, p+bef_data_len, buf_size-bef_data_len);
                            pes->data_index += buf_size+pes->pes_header_size;
                        }
                        else
                        {
                            memcpy(pes->buffer+pes->data_index, p, buf_size);
...
static void pmt_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    for(;;) {
        ...
        if(stream_type == STREAM_TYPE_AUDIO_MUPCM){
            //must set the audio codec info
            const uint8_t* desc_end;
            int desc_tag = get8(&p, p_end);
            int desc_len = get8(&p, p_end);
            int channel = 1;
            int sample_fmt = 1;
            int sample_rate = 8000;

            desc_end = p + desc_list_len -2;
            //av_log(NULL, AV_LOG_INFO,"%d, %d, %d, %d\n", desc_tag, desc_len, desc_list_len, desc_end-p_end);
            if(desc_list_len < 0 ||desc_end > p_end || desc_tag != AUDIO_DESCRIPTOR_TAG_MUPCM || desc_len != 5){
                av_log(NULL, AV_LOG_WARNING, "Lost descriptor for mu-law pcm audio stream. Set to default.\n");
                //st->codec
            }else{
                channel = get8(&p, p_end);
                sample_fmt = get8(&p, p_end);
                sample_rate = (get8(&p, p_end) << 16) |(get16(&p, p_end));
            }
            st->codec->channels = channel;
            st->codec->sample_fmt = sample_fmt;
            st->codec->sample_rate = sample_rate;
            //av_log(NULL, AV_LOG_INFO, "Channle: %d, sample_fmt:%d, sample_rate:%d.\n", channel, sample_fmt, sample_rate);
            st->request_probe = 0;
            desc_list_len -= 7;
        }
    ...
    ts->stream->mpegts_rtp_info_parsed = 1;
 out:
...
static void pat_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    for(;;) {
        ...
        if (sid == 0x0000) {
            ...
            if (ts->pids[pmt_pid]) {
                // if filter exists and is pmt section filter, then we need't close and reopen it.
                if ((ts->pids[pmt_pid]->type == MPEGTS_SECTION) && (ts->pids[pmt_pid]->u.section_filter.section_cb == pmt_cb)) {
                    break;
                }
