                  ...
                  int64_t (*seek)(void *opaque, int64_t offset, int whence))
{
    ...
    s->read_pause = NULL;
    s->read_seek  = NULL;
    s->seek_threshold = SHORT_SEEK_THRESHOLD;
    s->io_buff_size = IO_BUFFER_SIZE;
    return 0;
}
...
int64_t avio_seek(AVIOContext *s, int64_t offset, int whence)
{
    ...
    if (!s->must_flush &&
        ...
        s->buf_ptr = s->buffer + offset1;
               ...
               offset1 <= s->buf_end + s->seek_threshold - s->buffer) &&
...
static void fill_buffer(AVIOContext *s)
{
    ...
    int max_buffer_size = s->max_packet_size ? s->max_packet_size : s->io_buff_size;
...
#ifdef CONFIG_PRJA
int ffio_expand_buf_size(AVIOContext *s, int buf_size)
{
    uint8_t *buffer;
    buffer = av_malloc(buf_size);
    if (!buffer)
    ...
    int cur_pos = s->buf_ptr - s->buffer;

    av_log(NULL, AV_LOG_ERROR, "buffer=%p buf_ptr=%p buf_end=%p data_len=%d cur_pos=%d\n",
        s->buffer, s->buf_ptr, s->buf_end, data_len, cur_pos);

    memmove(buffer, s->buffer, data_len);
    av_free(s->buffer);

    s->buffer = buffer;
    s->buffer_size = buf_size;
    s->buf_ptr = buffer + cur_pos;
    s->buf_end = buffer + data_len;
    return 0;
}
#endif
