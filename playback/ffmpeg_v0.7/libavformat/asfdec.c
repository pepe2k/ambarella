...
static int asf_read_stream_properties(AVFormatContext *s, int64_t size)
{
    ...
    av_log(NULL, AV_LOG_DEBUG, "total_size=%llu\n", total_size);
...
static int asf_read_ext_stream_properties(AVFormatContext *s, int64_t size)
{
    ...
    uint32_t ext_d=0, leak_rate, stream_num;
    ...
    average_time_per_frame = avio_rl64(pb); // avg frametime in 100ns units
    if (stream_num < 128) {
        asf->streams[stream_num].average_time_per_frame = average_time_per_frame;
    }
    ...
    av_log(NULL, AV_LOG_DEBUG, "ext_d=%u\n", ext_d);
...
static int asf_read_metadata(AVFormatContext *s, int64_t size)
{
    ...
    for(i=0;i<n;i++) {
        ...
        av_log(NULL, AV_LOG_DEBUG, "value_type=%d\n", value_type);
        if ((ret = avio_get_str16le(pb, name_len, name, sizeof(name))) < name_len)
            avio_skip(pb, name_len - ret);
...
static int asf_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    for(;;) {
        ...
        if (!ff_guidcmp(&g, &ff_asf_file_header)) {
            ...
            v1 = avio_rl32(pb);
            v2 = avio_rl16(pb);
            av_log(NULL, AV_LOG_DEBUG, "v1=%d, v2=%d\n", v1, v2);
            continue;
...
//av_log(s, AV_LOG_INFO, "i=%d, st->codec->codec_type:%d, dar %d:%d sar=%d:%d\n", i, st->codec->codec_type, dar[i].num, dar[i].den, st->sample_aspect_ratio.num, st->sample_aspect_ratio.den);
            ...
            //GangLiu modified
            // stream_num(avid in AVFormatContext) is corresponding to i(asfid).
            st->average_time_per_frame = asf->streams[i].average_time_per_frame;
...
//            av_log(s, AV_LOG_DEBUG, "\n");
            ...
            ts0= avio_rl64(pb);
            ts1= avio_rl64(pb);
            av_log(NULL, AV_LOG_DEBUG, "ts1=%lld\n", ts1);
            avio_skip(pb, 12);
            avio_rl32(pb);
...
static int ff_asf_parse_packet(AVFormatContext *s, AVIOContext *pb, AVPacket *pkt)
{
    ...
    for (;;) {
        ...
        if (asf->packet_size_left < FRAME_HEADER_SIZE
            ...
            //assert(ret>=0); //remove this assert to avoid SEG-fault
            if(ret<0){
               av_log(s, AV_LOG_ERROR, "ff_asf_parse_packet error, ret=%d\n", ret);
               return AVERROR_EOF;
            }
            /* fail safe */
            avio_skip(pb, ret);
...
static int asf_read_seek(AVFormatContext *s, int stream_index, int64_t pts, int flags)
{
    ...
    if((asf->index_read && st->index_entries)){
	  ...
	  * Another modification in av_seek_frame(), utils.c. Need cooperation by demuxer filter to use seek by byte.
	  */
        if(st->nb_index_entries == 1){
            av_log(s, AV_LOG_DEBUG, "Has only 1 index entry, demux filter should seek by byte.\n");
            asf_reset_header(s);
            return -0xa5f;//asf
        }
...
/**
  * Add for pulldown streams, calculate the actual framerate
  * The encode fps may be different with the target framerate
  *
  * May only called for WVC1 streams
  */
int asf_read_adjust_fps(AVFormatContext *s, AVRational *actual_fps)
{
    AVPacket pkt1;
    AVIOContext *pb = s->pb;
    AVStream *st;
    ...
    old_offset = avio_tell(pb);

    for(;;){
        av_init_packet(&pkt1);
        err = asf_read_packet(s, &pkt1);
        if (err < 0) {
        ...
        st= s->streams[pkt1.stream_index];
        switch(st->codec->codec_type){
            case AVMEDIA_TYPE_VIDEO:
                //calculate the actral pts here:
                if(frames == 0){
                    first_timestamp = pkt1.dts;
                    timebase = st->time_base.den/st->time_base.num;//hz
                    av_log(s,AV_LOG_DEBUG,"get timebase = %lld.\n", timebase);
                }
                last_timestamp = pkt1.dts;
                frames++;
                break;
            default:
                break;
        }
        if((last_timestamp - first_timestamp) > timebase || eof){
            if(frames == 1){
                av_log(s,AV_LOG_WARNING,"Only got 1 frame.\n");
                break;
            }
            fps = ((frames-1) * timebase / (float)(last_timestamp - first_timestamp));
            //asf time_base = {1, 1000};
            actual_fps->num = st->time_base.den;
            actual_fps->den = st->time_base.num;
            if (fps> 9.990 && fps<=10.010){ fps=10.000; actual_fps->num *= 10; actual_fps->den *= 1000;}
            else if (fps>14.990 && fps<=15.010){ fps=15.000; actual_fps->num *= 15; actual_fps->den *= 1000;}
            else if (fps>23.964 && fps<=23.988){ fps=23.976; actual_fps->num *= 24; actual_fps->den *= 1001;}
            else if (fps>23.988 && fps<=24.012){ fps=24.000; actual_fps->num *= 24; actual_fps->den *= 1000;}
            else if (fps>24.988 && fps<=25.012){ fps=25.000; actual_fps->num *= 25; actual_fps->den *= 1000;}
            else if (fps>29.955 && fps<=29.985){ fps=29.970; actual_fps->num *= 30; actual_fps->den *= 1001;}
            else if (fps>29.985 && fps<=30.015){ fps=30.000; actual_fps->num *= 30; actual_fps->den *= 1000;}
            else if (fps>23.964*2 && fps<=23.988*2){ fps=23.976*2; actual_fps->num *= 48; actual_fps->den *= 1001;}
            else if (fps>23.988*2 && fps<=24.012*2){ fps=24.000*2; actual_fps->num *= 48; actual_fps->den *= 1000;}
            else if (fps>24.988*2 && fps<=25.012*2){ fps=25.000*2; actual_fps->num *= 50; actual_fps->den *= 1000;}
            else if (fps>29.955*2 && fps<=29.985*2){ fps=29.970*2; actual_fps->num *= 60; actual_fps->den *= 1001;}
            else if (fps>30.985*2 && fps<=30.015*2){ fps=30.000*2; actual_fps->num *= 60; actual_fps->den *= 1000;}
            else{
                av_log(s,AV_LOG_WARNING,"unknown fps, %f.\n",fps);
                fps0 = st->r_frame_rate.num /st->r_frame_rate.den;
                if((fps0 - fps) <0.3f && (fps0 - fps) >-0.3f){//close to the fps red from demux, do nothing
                    av_free_packet(&pkt1);
                    break;
                }
                actual_fps->num *= (frames-1);
                actual_fps->den *= (last_timestamp - first_timestamp);
            }
            av_log(s,AV_LOG_INFO,"update fps: frames=%d, time diff = %lld, actual fps = %2.3f.\n",
                 frames-1, last_timestamp-first_timestamp,fps);
            st->r_frame_rate.den = actual_fps->den;
            st->r_frame_rate.num = actual_fps->num;
            av_free_packet(&pkt1);
            break;
        }
        av_free_packet(&pkt1);
    ...
    avio_seek(pb, old_offset, SEEK_SET);
    return 0;
}
...
AVInputFormat ff_asf_demuxer = {
    ...
    asf_read_seek,
    asf_read_pts,
    .read_adjust_fps = asf_read_adjust_fps,
};
