...
static void mpegts_write_section(MpegTSSection *s, uint8_t *buf, int len)
{
    ...
    while (len > 0) {
        ...
        *q++ = b;
        *q++ = s->pid;
        s->cc = (s->cc + 1) & 0xf;//cc is continuity_counter
        *q++ = 0x10 | s->cc;
        if (first)
            *q++ = 0; /* 0 offset */       //Cannot understand this 0-offset--by cicai
...
#define DEFAULT_PROVIDER_NAME   "Ambarella iOne"
...
static void mpegts_write_pmt(AVFormatContext *s, MpegTSService *service)
{
    ...
    //no bug this length is for descriptors, we donot have any descriptors.
    ...
    for(i = 0; i < s->nb_streams; i++) {
        ...
        case CODEC_ID_PCM_MULAW:
            stream_type = STREAM_TYPE_AUDIO_MUPCM;
            break;
        default:
        ...
        *q++ = stream_type;
        ...
        case AVMEDIA_TYPE_AUDIO:
            if(st->codec->codec_id == CODEC_ID_PCM_MULAW){
                //AMB rtsp streaming audio codec info
                *q++ = AUDIO_DESCRIPTOR_TAG_MUPCM;
                *q++ = 5;
                *q++ = st->codec->channels;
                *q++ = st->codec->sample_fmt;
                *q++ = (st->codec->sample_rate >> 16);
                put16(&q, st->codec->sample_rate);
            }
                             ...
                             int64_t pts, int64_t dts)
{
    ...
    while (payload_size > 0) {
        ...
        *q++ = 0x10 | ts_st->cc | (write_pcr ? 0x20 : 0); //for adaptation_field_control, if(write_pcr) this field will be 11, else will be 01
        if (write_pcr) {
        ...
        if (stuffing_len > 0) { //this appear when the first es data < 188, header_len is copied to after afc+stuffing. then cpy the data.
        ...
        if(st->codec->codec_id == CODEC_ID_H264){
            //handle h264 here to save memory copy
            uint8_t startcode[3] = {0, 0, 1};

            avio_write(s->pb, buf, TS_PACKET_SIZE - len);//write header
            if(is_start){
                if (s->video_append_extradata) {
                    if (AV_RB32(payload) == 0x00000001) {
                        if (0x05 == (payload[4] & 0x1f)) {
                            av_log(s, AV_LOG_DEBUG, "user request append sequence data a, st->codec->extradata %p, st->codec->extradata_size %d\n", st->codec->extradata, st->codec->extradata_size);
                            if (st->codec->extradata) {
                                avio_write(s->pb, st->codec->extradata, st->codec->extradata_size);
                                len -= st->codec->extradata_size;
                            }
                        }
                    } else if (AV_RB24(payload) == 0x000001) {
                        if (0x05 == (payload[3] & 0x1f)) {
                            av_log(s, AV_LOG_DEBUG, "user request append sequence data b, st->codec->extradata %p, st->codec->extradata_size %d\n", st->codec->extradata, st->codec->extradata_size);
                            if (st->codec->extradata) {
                                avio_write(s->pb, st->codec->extradata, st->codec->extradata_size);
                                len -= st->codec->extradata_size;
                            }
                ...
                if (AV_RB32(payload) != 0x0000001 && AV_RB24(payload) != 0x000001){
                    av_log(s, AV_LOG_DEBUG, "h264 bitstream malformated, add startcode 0x000001.\n");
                    avio_write(s->pb, startcode, 3);
                    len -= 3;
                }

            }
            avio_write(s->pb, payload, len);
            payload += len;
            payload_size -= len;
        }else{
...
static int mpegts_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (st->codec->codec_id == CODEC_ID_H264) {
        ...
        //9->AUD, 5->slice of IDR, 1->slice of non-IDR, WHY?

        if ((state & 0x1f) != 9 && 0) { // AUD NAL //access unit delimiter, disable by cicai
