                          ...
                          PayloadContext *data, const char *line)
{
    ...
    }else if (av_strstart(line, "framesize:", &p)) {
        char buf1[50];
        char *dst = buf1;
    ...
    }else if(av_strstart(line, "framerate:", &p)){
        // a=framerate:8
        // set our parameters ...
        double frame_rate = atof(p);
        stream->r_frame_rate.num = codec->time_base.num = frame_rate * 100;
        stream->r_frame_rate.den = codec->time_base.den = 100;
    }
    return 0;
}
