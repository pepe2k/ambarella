...
#include "rv34_nv12.h"
#include "log_dump.h"
//keep the bug for chroma mv value 6 6 -> 4 4
//#define _bug_chroma_mv_6_6_4_4_
...
static DCTELEM* g_log_idct_data=NULL;
static DCTELEM* g_log_idct_data_uv=NULL;
static int g_log_idct_size=0;
static int g_log_idct_stride=0;
#endif
 ...
 */
static av_cold void rv34_nv12_init_tables(void)
{
    int i, j, k;
 ...
 */
static void rv34_nv12_inv_transform(DCTELEM *block){
    int temp[16];
    int i;
 ...
 *
 * Code is almost the same as rv34_nv12_inv_transform()
 * but final coefficients are multiplied by 1.5 and have no rounding.
 */
static void rv34_nv12_inv_transform_noround(DCTELEM *block){
    int temp[16];
    int i;
 ...
 *  o--o
 */
static inline void rv34_nv12_decode_block(DCTELEM *dst, GetBitContext *gb, RV34VLC *rvlc, int fc, int sc)
{
    int code, pattern;
 ...
 */
 #ifndef __commit_out_idct_dequant__
static inline void rv34_nv12_dequant4x4(DCTELEM *block, int Qdc, int Q)
{
    int i, j;
 ...
 * @todo optimize
 */
static inline void rv34_nv12_dequant4x4_16x16(DCTELEM *block, int Qdc, int Q)
{
    int i;
 ...
 */
int ff_rv34_nv12_get_start_offset(GetBitContext *gb, int mb_size)
{
    int i;
    for(i = 0; i < 5; i++)
...
static inline void rv34_nv12_mc(RV34DecContext *r, const int block_type,
                          ...
                          h264_chroma_mc_func (*chroma_mc))
{
    ...
    int is16x16 = 1;
        ...
        char tstr[100];
        int use_em=0;
        uint8_t* pt=dir ? s->next_picture_ptr->data[0] : s->last_picture_ptr->data[0];
        uint8_t* ptc=dir ? s->next_picture_ptr->data[1] : s->last_picture_ptr->data[1];
    #endif
    ...
    if(thirdpel){
        ...
        #ifndef _bug_chroma_mv_6_6_4_4_
        if(uvmx == 6 && uvmy == 6)
            uvmx = uvmy = 4;
        #endif
    }
    ...
    char txtch[80];
    snprintf(txtch,79,"[ mvx= %d , mvy= %d ]",s->current_picture_ptr->motion_val[dir][mv_pos][0],s->current_picture_ptr->motion_val[dir][mv_pos][1]);
    log_text(log_fd_dsp_text,txtch);
    snprintf(txtch,79,"[sub pixel luma: lx= %d , ly= %d; chroma: uvmx= %d ,uvmy= %d ]",lx,ly,uvmx,uvmy);
    log_text(log_fd_dsp_text,txtch);
    #endif
...
//        av_log(s->avctx, AV_LOG_ERROR, "****need s->dsp.emulated_edge_mc\n\n\n \n");
...
/*        s->dsp.emulated_edge_mc(uvbuf, srcU, s->uvlinesize, (width<<2)+1, (height<<2)+1,
        ...
        srcU = uvbuf;
        srcV = uvbuf + 16;*/
        s->dsp.emulated_edge_mc_nv12(uvbuf, srcU, s->uvlinesize, (width<<2)+1, (height<<2)+1,
                            uvsrc_x, uvsrc_y, s->h_edge_pos >> 1, s->v_edge_pos >> 1);
        srcU = uvbuf;
        srcV = uvbuf + 1;
    ...
    U = s->dest[1] + xoff + (yoff>>1)*s->uvlinesize;
    V = U+1;
    ...
    if(block_type == RV34_MB_P_16x8){
        ...
        #ifdef __log_mv__
            if(!use_em)
                snprintf(tstr,99,"y mc dxy=%d, des offset=%d, src offset=%d, dir=%d, size=1",dxy,Y-s->current_picture_ptr->data[0],srcY-pt,dir);
            else
                snprintf(tstr,99,"y mc_e dxy=%d, des offset=%d, src offset=%d, dir=%d, size=1",dxy,Y-s->current_picture_ptr->data[0],srcY-s->edge_emu_buffer,dir);
            log_text(log_fd_mv, tstr);
            snprintf(tstr,99,"y mv mv_x=%d, mv_y=%d; mx=%d,my=%d,lx=%d,ly=%d",s->current_picture_ptr->motion_val[dir][mv_pos][0],s->current_picture_ptr->motion_val[dir][mv_pos][1],mx,my,lx,ly);
            log_text(log_fd_mv, tstr);
            snprintf(tstr,99,"c mv cx=%d,cy=%d,umx=%d,umy=%d,uvmx=%d,uvmy=%d",s->current_picture_ptr->motion_val[dir][mv_pos][0]/2,s->current_picture_ptr->motion_val[dir][mv_pos][1]/2,umx,umy,uvmx,uvmy);
            log_text(log_fd_mv, tstr);
            if(dir)
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<P_16x8 first block, reference:>>");
        log_text_rect_char_hex(log_fd_dsp_text,srcY,8,8,s->linesize-8);
        #endif

        qpel_mc[1][dxy](Y, srcY, s->linesize);

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<MC result:>>");
        log_text_rect_char_hex(log_fd_dsp_text,Y,8,8,s->linesize-8);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<P_8x16 first block, reference:>>");
        log_text_rect_char_hex(log_fd_dsp_text,srcY,8,8,s->linesize-8);
        #endif
    ...
    is16x16 = (block_type != RV34_MB_P_8x8) && (block_type != RV34_MB_P_16x8) && (block_type != RV34_MB_P_8x16);
    ...
    #ifdef __log_mv__
        if(!use_em)
            snprintf(tstr,99,"y mc dxy=%d, des offset=%d, src offset=%d, dir=%d, size=%d",dxy,Y-s->current_picture_ptr->data[0],srcY-pt,dir,!is16x16);
        else
            snprintf(tstr,99,"y mc_e dxy=%d, des offset=%d, src offset=%d, dir=%d, size=%d",dxy,Y-s->current_picture_ptr->data[0],srcY-s->edge_emu_buffer,dir,!is16x16);
        log_text(log_fd_mv, tstr);
        if(!use_em)
            snprintf(tstr,99,"c mc  des offset=%d, src offset=%d",U-s->current_picture_ptr->data[1],srcU-ptc);
        else
        ...
        {
            char pmbstr[80];
            snprintf(pmbstr,79,"MBmcsrc2_x=%d_y=%d_%d_nv12",s->mb_x,s->mb_y,dir);
    ...
    if (block_type == RV34_MB_P_16x8)
    {
        ambadec_assert_ffmpeg(!is16x16);
        ambadec_assert_ffmpeg(height==1);
        ambadec_assert_ffmpeg(width==2);

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<P_16x8 second block, reference:>>");
        log_text_rect_char_hex(log_fd_dsp_text,srcY,8,8,s->linesize-8);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<P_8x4 U: reference:>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,srcU,8,4,s->uvlinesize-16);
        #endif

        chroma_mc[2-width]   (U, srcU, s->uvlinesize, height*4, uvmx, uvmy);

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<MC U: result:>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,U,8,4,s->uvlinesize-16);
        #endif

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<P_8x4 V: reference:>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,srcV,8,4,s->uvlinesize-16);
        #endif

        chroma_mc[2-width]   (V, srcV, s->uvlinesize, height*4, uvmx, uvmy);

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<MC V: result:>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,V,8,4,s->uvlinesize-16);
        #endif
    ...
    {
        ambadec_assert_ffmpeg(!is16x16);
        ambadec_assert_ffmpeg(height==2);
        ambadec_assert_ffmpeg(width==1);

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<P_8x16 second block, reference:>>");
        log_text_rect_char_hex(log_fd_dsp_text,srcY,8,8,s->linesize-8);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<P_4x8 U: reference:>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,srcU,4,8,s->uvlinesize-8);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<MC U: result:>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,U,4,8,s->uvlinesize-8);
        #endif

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<P_4x8 V: reference:>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,srcV,4,8,s->uvlinesize-8);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<MC V: result:>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,V,4,8,s->uvlinesize-8);
        #endif
    }
    else if(block_type == RV34_MB_P_8x8)
    {
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<P_8x8 block, reference:>>");
        log_text_rect_char_hex(log_fd_dsp_text,srcY,8,8,s->linesize-8);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<P_4x4 U: reference:>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,srcU,4,4,s->uvlinesize-8);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<MC U: result:>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,U,4,4,s->uvlinesize-8);
        #endif

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<P_4x4 V: reference:>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,srcV,4,4,s->uvlinesize-8);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<MC V: result:>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,V,4,4,s->uvlinesize-8);
        #endif
    }
    else
    {
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<16x16 block, reference:>>");
        log_text_rect_char_hex(log_fd_dsp_text,srcY,16,16,s->linesize-16);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<MC result:>>");
        log_text_rect_char_hex(log_fd_dsp_text,Y,16,16,s->linesize-16);
        #endif

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<P_8x8 U: reference:>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,srcU,8,8,s->uvlinesize-16);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<MC U: result:>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,U,8,8,s->uvlinesize-16);
        #endif

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<P_8x8 V: reference:>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,srcV,8,8,s->uvlinesize-16);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<MC V: result:>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,V,8,8,s->uvlinesize-16);
        #endif
    }
#else
    qpel_mc[!is16x16][dxy](Y, srcY, s->linesize);
...
static void rv34_nv12_mc_1mv(RV34DecContext *r, const int block_type,
                        const int xoff, const int yoff, int mv_off,
                        const int width, const int height, int dir)
{
    #ifdef __dump_DSP_TXT__
    if(dir)
        log_text(log_fd_dsp_text," Backward reference.");
    else
        log_text(log_fd_dsp_text," Forward reference.");
    log_text(log_fd_dsp_text," MC put pixel value: ");
    #endif
    rv34_nv12_mc(r, block_type, xoff, yoff, mv_off, width, height, dir, r->rv30,
            r->rv30 ? r->s.dsp.put_rv30_tpel_pixels_tab
                    : r->s.dsp.put_rv40_qpel_pixels_tab,
            r->rv30 ? r->s.dsp.put_h264_chroma_pixels_tab_nv12
                    : r->s.dsp.put_rv40_chroma_pixels_tab_nv12);
}

static void rv34_nv12_mc_2mv(RV34DecContext *r, const int block_type)
{
    #ifdef __dump_DSP_TXT__
    log_text(log_fd_dsp_text," Forward reference.");
    log_text(log_fd_dsp_text," MC put pixel value: ");
    #endif
    rv34_nv12_mc(r, block_type, 0, 0, 0, 2, 2, 0, r->rv30,
            r->rv30 ? r->s.dsp.put_rv30_tpel_pixels_tab
                    : r->s.dsp.put_rv40_qpel_pixels_tab,
            r->rv30 ? r->s.dsp.put_h264_chroma_pixels_tab_nv12
                    : r->s.dsp.put_rv40_chroma_pixels_tab_nv12);
    #ifdef __dump_DSP_TXT__
    log_text(log_fd_dsp_text," Backward reference.");
    log_text(log_fd_dsp_text," MC add pixel value: ");
    #endif
    rv34_nv12_mc(r, block_type, 0, 0, 0, 2, 2, 1, r->rv30,
            r->rv30 ? r->s.dsp.avg_rv30_tpel_pixels_tab
                    : r->s.dsp.avg_rv40_qpel_pixels_tab,
            r->rv30 ? r->s.dsp.avg_h264_chroma_pixels_tab_nv12
                    : r->s.dsp.avg_rv40_chroma_pixels_tab_nv12);
}

static void rv34_nv12_mc_2mv_skip(RV34DecContext *r)
{
    int i, j;
    for(j = 0; j < 2; j++)
        for(i = 0; i < 2; i++){
        ...
        log_text(log_fd_dsp_text," MC put pixel value: ");
        #endif
             rv34_nv12_mc(r, RV34_MB_P_8x8, i*8, j*8, i+j*r->s.b8_stride, 1, 1, 0, r->rv30,
                    r->rv30 ? r->s.dsp.put_rv30_tpel_pixels_tab
                            : r->s.dsp.put_rv40_qpel_pixels_tab,
                    r->rv30 ? r->s.dsp.put_h264_chroma_pixels_tab_nv12
                            : r->s.dsp.put_rv40_chroma_pixels_tab_nv12);
        ...
        log_text(log_fd_dsp_text," MC add pixel value: ");
        #endif
             rv34_nv12_mc(r, RV34_MB_P_8x8, i*8, j*8, i+j*r->s.b8_stride, 1, 1, 1, r->rv30,
                    r->rv30 ? r->s.dsp.avg_rv30_tpel_pixels_tab
                            : r->s.dsp.avg_rv40_qpel_pixels_tab,
                    r->rv30 ? r->s.dsp.avg_h264_chroma_pixels_tab_nv12
                            : r->s.dsp.avg_rv40_chroma_pixels_tab_nv12);
 ...
 */
static int rv34_nv12_decode_mv(RV34DecContext *r, int block_type)
{
    MpegEncContext *s = &r->s;
    ...
    case RV34_MB_SKIP:
        if(s->pict_type == FF_P_TYPE){
            ...
            #ifdef __dump_DSP_TXT__
            log_text(log_fd_dsp_text,"<<<<< MC SKIP MB: >>>>>");
            #endif
            ...
            #endif
            rv34_nv12_mc_1mv (r, block_type, 0, 0, 0, 2, 2, 0);
            #endif
            break;
    ...
    case RV34_MB_B_DIRECT:
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<<<< MC B_DIRECT MB: >>>>>");
        #endif
    ...
    case RV34_MB_P_MIX16x16:
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<<<< MC P_16x16 or P_MIX16x16 MB: >>>>>");
        #endif
    ...
    case RV34_MB_B_BACKWARD:
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<<<< MC B_FWD or B_BWD MB: >>>>>");
        #endif
        ...
        if(log_mask[decoding_config_mc])
        #endif
        rv34_nv12_mc_1mv     (r, block_type, 0, 0, 0, 2, 2, block_type == RV34_MB_B_BACKWARD);
        #endif
        break;
    case RV34_MB_P_16x8:
    case RV34_MB_P_8x16:
        ...
        if(block_type == RV34_MB_P_16x8){
            ...
            #ifdef __dump_DSP_TXT__
            log_text(log_fd_dsp_text,"<<<<< MC P_16x8 MB: >>>>>");
            #endif
            ...
            if(log_mask[decoding_config_mc])
            #endif
            {rv34_nv12_mc_1mv(r, block_type, 0, 0, 0,            2, 1, 0);
            rv34_nv12_mc_1mv(r, block_type, 0, 8, s->b8_stride, 2, 1, 0);}
            #endif
        }
        if(block_type == RV34_MB_P_8x16){

            #ifdef __dump_DSP_TXT__
            log_text(log_fd_dsp_text,"<<<<< MC P_8x16 MB: >>>>>");
            #endif
            ...
            if(log_mask[decoding_config_mc])
            #endif
            {rv34_nv12_mc_1mv(r, block_type, 0, 0, 0, 1, 2, 0);
            rv34_nv12_mc_1mv(r, block_type, 8, 0, 1, 1, 2, 0);}
            #endif
        }
        break;
    case RV34_MB_B_BIDIR:
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<<<< MC B_BIDIR MB: >>>>>");
        #endif
    ...
    case RV34_MB_P_8x8:
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<<<< MC P_8x8 MB: >>>>>");
        #endif
        ...
        for(i=0;i< 4;i++){
            ...
            if(log_mask[decoding_config_mc])
            #endif
            rv34_nv12_mc_1mv (r, block_type, (i&1)<<3, (i&2)<<2, (i&1)+(i>>1)*s->b8_stride, 1, 1, 0);
            #endif
        }
        break;
    }
 ...
 */
 static void rv34_pred_4x4_block_y(RV34DecContext *r, uint8_t *dst, int stride, int itype, int up, int left, int down, int right)
{
    ...
    if(!right && up){
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"use top left");
        #endif
    }
    #ifdef __log_intrapred__
        char tstr[40];
        snprintf(tstr,39,"y type=%d, offset=%d ",itype,(dst-r->s.current_picture_ptr->data[0]));
        log_text(log_fd_intrapred, tstr);
    #endif

    #ifdef __dump_DSP_TXT__
    char txt[20];
    snprintf(txt,19,"intra pred type=%d",itype);
    log_text(log_fd_dsp_text,txt);
    #endif

    r->h.pred4x4[itype](dst, prev, stride);
}

static void rv34_pred_4x4_block_nv12(RV34DecContext *r, uint8_t *dst, int stride, int itype, int up, int left, int down, int right)
{
    uint8_t *prev = dst - stride + 8;
    uint64_t topleft;
#ifdef __dump_DSP_TXT__
    int use_topleft = 0;
#endif
    ...
    if(!right && up){
        topleft = dst[-stride + 6] * 0x0001000100010001ULL+dst[-stride + 7]*0x0100010001000100ULL;
        prev = (uint8_t*)&topleft;
        #ifdef __log_intrapred__
    ...
    #ifdef __log_intrapred__
        char tstr[40];
        snprintf(tstr,39,"uv type=%d, offset=%d ",itype,(dst-r->s.current_picture_ptr->data[1]));
        log_text(log_fd_intrapred, tstr);
    #endif
/*    #ifdef __log_decoding_config__
    if((itype<8) ||(itype>7 && log_mask[itype-8]))
    #endif*/
    #ifdef __dump_DSP_TXT__
    char txt[20];
    snprintf(txt,19,"intra pred type=%d",itype);
    ...
    r->h.pred4x4_nv12[itype](dst, prev, stride);
}

/** add_pixels_clamped for 4x4 block */
static void rv34_add_4x4_block_y(uint8_t *dst, int stride, DCTELEM block[64], int off)
{
    int x, y;
    for(y = 0; y < 4; y++)
        for(x = 0; x < 4; x++)
...
/** add_pixels_clamped for 4x4 block */
static void rv34_add_4x4_block_nv12(uint8_t *dst, int stride, DCTELEM block[64], int off)
{
    int x, y;
    for(y = 0; y < 4; y++)
        for(x = 0; x < 4; x++)
            dst[(x<<1) + y*stride] = av_clip_uint8(dst[(x<<1) + y*stride] + block[off + x+y*8]);
}
...
static void rv34_nv12_output_macroblock(RV34DecContext *r, int8_t *intra_types, int cbp, int is16)
{
    MpegEncContext *s = &r->s;
    ...
    #ifdef __log_intrapred__
        char tstr[40];
        snprintf(tstr,39,"MBintrapred_x=%d_y=%d ",s->mb_x,s->mb_y);
        log_text(log_fd_intrapred, tstr);
    #endif
    ...
    if(!is16){
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<<<< Luma 4x4 Intra Prediction : >>>>>");
        #endif
        ...
        for(j = 0; j < 4; j++){
            ...
            for(i = 0; i < 4; i++, cbp >>= 1, Y += 4, idx++){
                ...
                #ifdef __log_intrapred__
                    char tstr[40];
                    snprintf(tstr,39,"y index=%d ",j*4+i);
                    log_text(log_fd_intrapred, tstr);
                #endif
                ...
                #endif
                {
                    rv34_pred_4x4_block_y(r, Y, s->linesize, ittrans[intra_types[i]], avail[idx-8], avail[idx-1], avail[idx+7], avail[idx-7]);
                    #ifdef __dump_DSP_TXT__
                    log_text(log_fd_dsp_text,"<<after intra pred>>");
                    log_text_rect_char_hex(log_fd_dsp_text,Y,4,4,s->linesize-4);
                    #endif
                }
                avail[idx] = 1;
                #ifdef __log_decoding_config__
                ...
                {if(cbp & 1)
                  {
                    #ifdef _config_rv40_neon_
                    r->neon.y4x4_addresidue(Y,&s->block[(i>>1)+(j&2)][(i&1)*4+(j&1)*32] ,s->linesize);
                    #else
                    rv34_add_4x4_block_y(Y, s->linesize, s->block[(i>>1)+(j&2)], (i&1)*4+(j&1)*32);
                    #endif
                    }
                }

                #ifdef __dump_DSP_TXT__
                    log_text(log_fd_dsp_text,"<<after add reverse transform>>");
                    log_text_rect_char_hex(log_fd_dsp_text,Y,4,4,s->linesize-4);
                #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<<<< Chroma 4x4 Intra Prediction : >>>>>");
        #endif
        for(j = 0; j < 2; j++){
            idx = 6 + j*4;
            for(i = 0; i < 2; i++, cbp >>= 1, idx++){
                ...
                #ifdef __log_intrapred__
                    char tstr[40];
                    snprintf(tstr,39,"uv index=%d ",(j<<1)+i+16);
                    log_text(log_fd_intrapred, tstr);
                #endif
                ...
                if(log_mask[decoding_config_intrapred])
                    ...
                    {
                   // if(log_mask[14])
                rv34_pred_4x4_block_nv12(r, U + (i<<3) + j*4*s->uvlinesize, s->uvlinesize, ittrans[intra_types[i*2+j*2*r->intra_types_stride]], r->avail_cache[idx-4], r->avail_cache[idx-1], !i && !j, r->avail_cache[idx-3]);
//                rv34_pred_4x4_block_nv12(r, V + (i<<3) + j*4*s->uvlinesize, s->uvlinesize, ittrans[intra_types[i*2+j*2*r->intra_types_stride]], r->avail_cache[idx-4], r->avail_cache[idx-1], !i && !j, r->avail_cache[idx-3]);
                #ifdef __dump_DSP_TXT__
                log_text(log_fd_dsp_text,"<<after intra pred U>>");
                log_text_rect_char_hex_chroma(log_fd_dsp_text,U + (i<<3) + j*4*s->uvlinesize,4,4,s->uvlinesize-8);
                log_text(log_fd_dsp_text,"<<after intra pred V>>");
                log_text_rect_char_hex_chroma(log_fd_dsp_text,V + (i<<3) + j*4*s->uvlinesize,4,4,s->uvlinesize-8);
                #endif
                }
                r->avail_cache[idx] = 1;
                #ifdef __log_decoding_config__
                ...
                {
                    #ifdef _config_rv40_neon_
                    r->neon.uv4x4_addresidue(U + (i<<3) + j*4*s->uvlinesize,&s->block[4][i*4+j*32] ,s->uvlinesize);
                    #else
                    rv34_add_4x4_block_nv12(U + (i<<3) + j*4*s->uvlinesize, s->uvlinesize, s->block[4], i*4+j*32);
                    #endif
                }
                if(cbp & 0x10)
                {
                    #ifdef _config_rv40_neon_
                    r->neon.uv4x4_addresidue(V + (i<<3) + j*4*s->uvlinesize,&s->block[5][i*4+j*32] ,s->uvlinesize);
                    #else
                    rv34_add_4x4_block_nv12(V + (i<<3) + j*4*s->uvlinesize, s->uvlinesize, s->block[5], i*4+j*32);
                    #endif
                }
                }
                #ifdef __dump_DSP_TXT__
                log_text(log_fd_dsp_text,"<<after add reverse transform U>>");
                log_text_rect_char_hex_chroma(log_fd_dsp_text,U + (i<<3) + j*4*s->uvlinesize,4,4,s->uvlinesize-8);
                log_text(log_fd_dsp_text,"<<after add reverse transform V>>");
                log_text_rect_char_hex_chroma(log_fd_dsp_text,V + (i<<3) + j*4*s->uvlinesize,4,4,s->uvlinesize-8);
                #endif
            }
        }
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<<<< Luma 16x16 Intra Prediction : >>>>>");
        snprintf(txtch,79,"intra pred type=%d",itype);
        log_text(log_fd_dsp_text,txtch);
        #endif
            ...
            snprintf(tstr,39,"y type=%d, offset=%d ",itype,Y-s->current_picture_ptr->data[0]);
            log_text(log_fd_intrapred, tstr);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<<<< Chroma 8x8 Intra Prediction : >>>>>");
        snprintf(txtch,79,"intra pred type=%d",itype);
        log_text(log_fd_dsp_text,txtch);
        #endif

//        log_dump(2, &itype,sizeof(int));
        #ifdef __log_intrapred__
            log_text(log_fd_intrapred, "uv index=1 ");
            snprintf(tstr,39,"uv type=%d, offset=%d ",itype,U-s->current_picture_ptr->data[1]);
            log_text(log_fd_intrapred, tstr);
        #endif
        ...
        #endif
        {//if(log_mask[itype])
        r->h.pred8x8_nv12[itype](U, s->uvlinesize);}

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<after intra pred U>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,U,8,8,s->linesize-16);
        log_text(log_fd_dsp_text,"<<after intra pred V>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,U+1,8,8,s->linesize-16);
        #endif
        ...
        if(log_mask[decoding_config_add_idct])
        #endif
        {dsp->add_pixels_clamped_chroma_nv12(s->block[4], U, s->uvlinesize);
//        if(log_mask[itype])
//            r->h.pred8x8_nv12[itype](V, s->uvlinesize);
        dsp->add_pixels_clamped_chroma_nv12(s->block[5], V, s->uvlinesize);}
        #else
        ambadec_assert_ffmpeg(s->linesize==s->uvlinesize);
        r->neon.mb_addresidue(Y,U ,s->block,s->linesize);
        #endif

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<after add residue U>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,U,8,8,s->linesize-16);
        log_text(log_fd_dsp_text,"<<after add residue V>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,U+1,8,8,s->linesize-16);
        #endif
 ...
 */
static int rv34_nv12_decode_mb_header(RV34DecContext *r, int8_t *intra_types)
{
    MpegEncContext *s = &r->s;
    GetBitContext *gb = &s->gb;
    int mb_pos = s->mb_x + s->mb_y * s->mb_stride;
    ...
    if(!r->si.type){
        ...
        rv34_nv12_decode_mv(r, r->block_type);
        if(r->block_type == RV34_MB_SKIP){
...
static void rv34_nv12_apply_differences(RV34DecContext *r, int cbp)
{
    static const int shifts[4] = { 0, 2, 8, 10 };
    MpegEncContext *s = &r->s;
    int i;
    ...
    #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<<<< total MC result Y: >>>>>");
        log_text_rect_char_hex(log_fd_dsp_text,s->dest[0],16,16,s->linesize-16);
        log_text(log_fd_dsp_text,"<<<<< total MC result U: >>>>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,s->dest[1],8,8,s->uvlinesize-16);
        log_text(log_fd_dsp_text,"<<<<< total MC result V: >>>>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,s->dest[1]+1,8,8,s->uvlinesize-16);
    #endif
    ...
    {
    #ifdef _config_rv40_neon_
    r->neon.mb_addresidue(s->dest[0],s->dest[1],s->block,s->linesize);
    #else
    {
        for(i = 0; i < 4; i++)
        ...
        if(cbp & U_CBP_MASK)
            s->dsp.add_pixels_clamped_chroma_nv12(s->block[4], s->dest[1], s->uvlinesize);
        if(cbp & V_CBP_MASK)
            s->dsp.add_pixels_clamped_chroma_nv12(s->block[5], s->dest[1]+1, s->uvlinesize);
    }
    #endif
    }
}
...
static int rv34_nv12_set_deblock_coef(RV34DecContext *r)
{
    MpegEncContext *s = &r->s;
...
static int rv34_nv12_decode_macroblock(RV34DecContext *r, int8_t *intra_types)
{
    MpegEncContext *s = &r->s;
    GetBitContext *gb = &s->gb;
    int cbp, cbp2;
    ...
    DCTELEM* plogy=g_log_idct_data+(s->mb_x<<8);
    DCTELEM* ploguv=g_log_idct_data_uv+(s->mb_x<<7);
    DCTELEM* ptmp=plogy;
    #endif
    ...
    #ifdef __log_mv__
        char strttt[40];
        snprintf(strttt,39,"MB_x=%d_y=%d ",s->mb_x,s->mb_y);
        log_text(log_fd_mv, strtt);
    #endif
    ...
    s->qscale = r->si.quant;
    cbp = cbp2 = rv34_nv12_decode_mb_header(r, intra_types);
    r->cbp_luma  [mb_pos] = cbp;
    r->cbp_chroma[mb_pos] = cbp >> 16;
    if(s->pict_type == FF_I_TYPE)
        r->deblock_coefs[mb_pos] = 0xFFFF;
    else
        r->deblock_coefs[mb_pos] = rv34_nv12_set_deblock_coef(r) | r->cbp_luma[mb_pos];
    ...
    luma_dc_quant = r->block_type == RV34_MB_P_MIX16x16 ? r->luma_dc_quant_p[s->qscale] : r->luma_dc_quant_i[s->qscale];
        ...
        log_text(log_fd_dsp_text,"<<<<< Inverse transform & DeQuant >>>>>");

    #endif

    if(r->is16){
        memset(block16, 0, sizeof(block16));
        rv34_nv12_decode_block(block16, gb, r->cur_vlcs, 3, 0);
        ...
        #endif
        {
        rv34_nv12_dequant4x4_16x16(block16, rv34_qscale_tab[luma_dc_quant],rv34_qscale_tab[s->qscale]);
        ...
        #else
        rv34_nv12_inv_transform_noround(block16);
        #endif
    ...
    for(i = 0; i < 16; i++, cbp >>= 1){
        ...
        if(cbp & 1)
        {
            rv34_nv12_decode_block(s->block[blknum] + blkoff, gb, r->cur_vlcs, r->luma_vlc, 0);

            #ifdef __dump_DSP_TXT__
            log_text(log_fd_dsp_text,"<< Before Inverse Quantization: >>");
            log_text_rect_short(log_fd_dsp_text, s->block[blknum] + blkoff, 4, 4, 4);
            #endif
        ...
        #endif
        {
        rv34_nv12_dequant4x4(s->block[blknum] + blkoff, rv34_qscale_tab[s->qscale],rv34_qscale_tab[s->qscale]);
        ...
        #ifdef _config_rv40_neon_
        r->neon.trans[trans_type_8x8](s->block[blknum] + blkoff);
        #else
        rv34_nv12_inv_transform(s->block[blknum] + blkoff);
        #endif

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<< After Inverse transform: >>");
        log_text_rect_short_hex(log_fd_dsp_text, s->block[blknum] + blkoff, 4, 4, 4);
        #endif
    ...
    for(; i < 24; i++, cbp >>= 1){
        ...
        rv34_nv12_decode_block(s->block[blknum] + blkoff, gb, r->cur_vlcs, r->chroma_vlc, 1);
        ...
        #endif
        {
        rv34_nv12_dequant4x4(s->block[blknum] + blkoff, rv34_qscale_tab[rv34_chroma_quant[1][s->qscale]],rv34_qscale_tab[rv34_chroma_quant[0][s->qscale]]);
    ...
    {
        for(i=0;i<6;i++)
            log_idct_txt(log_fd_idct_text,s->block[i],8);
    }
    //fill y
    for(i=0;i<4;)
    {
        for(j=0;j<8;j++)
        {
            memcpy(ptmp,&s->block[i][j<<3],16);
            ptmp+=16;
        }
        i++;
        ptmp=plogy+((i&0x1)<<3)+((i&0x2)<<6);
    }
    //fill uv
    ptmp=ploguv;
    for(i=0;i<2;)
    ...
    //dump after mc
        ...
        {
            char pmbstr[40];
            snprintf(pmbstr,39,"MBmc_x=%d_y=%d_nv12",s->mb_x,s->mb_y);
            log_openfile(log_fd_mb_data,pmbstr);
            log_dump_rect(log_fd_mb_data, s->current_picture_ptr->data[0]+(s->mb_x<<4)+(s->mb_y<<4)*s->linesize, 16, 16,s->linesize);
            log_dump_rect(log_fd_mb_data, s->current_picture_ptr->data[1]+(s->mb_x<<4)+(s->mb_y<<3)*s->uvlinesize, 16, 8,s->uvlinesize);
            log_closefile(log_fd_mb_data);
        }
    #endif
    ...
    #ifndef __remove_memory_op__
    {
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<<<< total inverse transform result: >>>>>");
        log_text(log_fd_dsp_text,"Y 0:");
        log_text_rect_short_hex(log_fd_dsp_text,s->block[0],8,8,0);
        log_text(log_fd_dsp_text,"Y 1:");
        log_text_rect_short_hex(log_fd_dsp_text,s->block[1],8,8,0);
        log_text(log_fd_dsp_text,"Y 2:");
        log_text_rect_short_hex(log_fd_dsp_text,s->block[2],8,8,0);
        log_text(log_fd_dsp_text,"Y 3:");
        log_text_rect_short_hex(log_fd_dsp_text,s->block[3],8,8,0);
        log_text(log_fd_dsp_text,"<< inverse transform result U: >>");
        log_text_rect_short_hex(log_fd_dsp_text,s->block[4],8,8,0);
        log_text(log_fd_dsp_text,"<< inverse transform result V: >>");
        log_text_rect_short_hex(log_fd_dsp_text,s->block[5],8,8,0);
        #endif
    ...
    if(IS_INTRA(s->current_picture_ptr->mb_type[mb_pos]))
    {
        ...
        #endif
        rv34_nv12_output_macroblock(r, intra_types, cbp2, r->is16);
    }
    else
    {
        #ifdef __dump_DSP_test_data__
            ...
            log_dump(log_fd_mbtype,&logmbtype,1);
        #endif
        rv34_nv12_apply_differences(r, cbp2);
    }

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<<<< before deblock Y: >>>>>");
        log_text_rect_char_hex(log_fd_dsp_text,s->dest[0],16,16,s->linesize-16);
        log_text(log_fd_dsp_text,"<<<<< before deblock U: >>>>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,s->dest[1],8,8,s->uvlinesize-16);
        log_text(log_fd_dsp_text,"<<<<< before deblock V: >>>>>");
        log_text_rect_char_hex_chroma(log_fd_dsp_text,s->dest[1]+1,8,8,s->uvlinesize-16);
        #endif
    }
    #endif

    //dump after intrapred and add inverse transform result
    #ifdef __dump_mb_data__
        if(s->mb_x>=log_start_mb_x && s->mb_x<=log_end_mb_x && s->mb_y>=log_start_mb_y && s->mb_y<=log_end_mb_y )
        {
            char pmbstr[40];
            snprintf(pmbstr,39,"MBadd_x=%d_y=%d_nv12",s->mb_x,s->mb_y);
            log_openfile(log_fd_mb_data,pmbstr);
            log_dump_rect(log_fd_mb_data, s->current_picture_ptr->data[0]+(s->mb_x<<4)+(s->mb_y<<4)*s->linesize, 16, 16,s->linesize);
            log_dump_rect(log_fd_mb_data, s->current_picture_ptr->data[1]+(s->mb_x<<4)+(s->mb_y<<3)*s->uvlinesize, 16, 8,s->uvlinesize);
            log_closefile(log_fd_mb_data);
...
static int check_slice_end_nv12(RV34DecContext *r, MpegEncContext *s)
{
    int bits;
...
static int rv34_nv12_decode_slice(RV34DecContext *r, int end, const uint8_t* buf, int buf_size)
{
    MpegEncContext *s = &r->s;
    GetBitContext *gb = &s->gb;
    int mb_pos;
    ...
    while(!check_slice_end_nv12(r, s)) {
        ff_update_block_index_nv12(s);
        s->dsp.clear_blocks(s->block[0]);

        if(rv34_nv12_decode_macroblock(r, r->intra_types + s->mb_x * 4 + 4) < 0){
    ...
    if(g_log_idct_data)
    {
        log_dump(log_fd_idct_y, g_log_idct_data,(g_log_idct_size<<1));
        log_dump(log_fd_idct_uv, g_log_idct_data_uv,g_log_idct_size);

        memset(g_log_idct_data,0x0,g_log_idct_size*3);
    }
#endif
...
#ifndef __commit_out_deblocking__
            #ifdef __log_decoding_config__
 ...
 * Initialize decoder.
 */
av_cold int ff_rv34_nv12_decode_init(AVCodecContext *avctx)
{
...
//#endif
    ...
    if(!intra_vlcs[0].cbppattern[0].bits)
        rv34_nv12_init_tables();

#ifdef __log_dump_idct_data__
    g_log_idct_size=(r->s.mb_width<<7)*sizeof(DCTELEM);
    g_log_idct_data=av_malloc((g_log_idct_size<<1)+g_log_idct_size);
    if(g_log_idct_data)
    {
        memset(g_log_idct_data,0x0,g_log_idct_size*3);
        g_log_idct_data_uv=g_log_idct_data+(r->s.mb_width<<8);
        g_log_idct_stride=r->s.mb_width<<4;
    }
#endif
...
{
//    short x=0x1234;
//    av_log(NULL,AV_LOG_INFO,"little endian or big endian, %x.\n",*((char*)(&x)));
    av_log(NULL,AV_LOG_INFO,"buf+n*8[0]=%x,[1]=%x,[2]=%x,[3]=%x.\n",*((char*)(buf+n*8)),*((char*)(buf+n*8+1)),*((char*)(buf+n*8+2)),*((char*)(buf+n*8+3)));
    av_log(NULL,AV_LOG_INFO,"buf+n*8[-4]=%x,[-3]=%x,[-2]=%x,[-1]=%x.\n",*((char*)(buf+n*8-4)),*((char*)(buf+n*8-3)),*((char*)(buf+n*8-2)),*((char*)(buf+n*8-1)));

    if(avctx->slice_count){
        av_log(NULL,AV_LOG_INFO,"return1 %d.\n",avctx->slice_offset[n]);
        return avctx->slice_offset[n];
        }
    else{
        av_log(NULL,AV_LOG_INFO," AV_RL32(buf + n*8 - 4)=%d.\n",AV_RL32(buf + n*8 - 4));
                av_log(NULL,AV_LOG_INFO," return2 %d.\n",(AV_RL32(buf + n*8 - 4) == 1 ? AV_RL32(buf + n*8) :  AV_RB32(buf + n*8)));
...
static int get_slice_offset(AVCodecContext *avctx, const uint8_t *buf, int n)
{
    ...
    if (!buf)
        snprintf(dump_str,99,"start %d get slice offset, buf NULL, n %d, p_vld->slice_cnt %d", cnt++, n, avctx->slice_count);
    else
        snprintf(dump_str,99,"start %d get slice offset, n %d, p_vld->slice_cnt %d", cnt++, n, avctx->slice_count);

    log_text_f(log_fd_temp, dump_str);

    if(avctx->slice_count)
    {
        ret = avctx->slice_offset[n];
        snprintf(dump_str,99, "1have slice_cnt %d, ret %d.\n", avctx->slice_count, ret);
        log_text_f(log_fd_temp, dump_str);
    }
    else {
...
int ff_rv34_nv12_decode_frame(AVCodecContext *avctx,
                            ...
                            AVPacket *avpkt)
{
    ...
    if (buf_size == 0) {
        ...
        char dump_str[100];
        int itt ;
        snprintf(dump_str,99,"p_vld->slice_cnt %d", avctx->slice_count);
        log_text_f(log_fd_temp, dump_str);
        ...
        if(!avctx->slice_count){
            ...
            for (itt = 0; itt<slice_count; itt++) {
                snprintf(dump_str,99,"%d p_vld->slice_offset %d", itt, avctx->slice_offset[itt]);
                log_text_f(log_fd_temp, dump_str);
            }
        }
#else
    ...
    offset = get_slice_offset(avctx, slices_hdr, 0) ;
...
#ifdef __log_dump_idct_data__
log_openfile(log_fd_idct_y,"idct_data_nv12_y");
log_openfile(log_fd_idct_uv,"idct_data_nv12_uv");
//log_openfile_text(log_fd_idct_text,"idct_text_nv12");
#endif
#ifdef __log_intrapred__
log_openfile(log_fd_intrapred, "nv12/intrapred_type");
#endif
#ifdef __log_mc__
log_openfile(log_fd_mc, "nv12/mc_type");
#endif
#ifdef __log_mv__
log_openfile(log_fd_mv, "nv12/mv_value");
#endif
    ...
    log_openfile_text(log_fd_dsp_deblock_text,"rv_deblock_text");
    ...
    for(i=0; i<slice_count; i++){
        ...
        offset_1 = get_slice_offset(avctx, slices_hdr, i+1);
        int size;
        if(i+1 == slice_count)
        ...
        last = rv34_nv12_decode_slice(r, r->si.end, buf + offset, size);
    ...
    if(last){
        ...
        AVFrame* pdecpic = NULL;
        if (s->pict_type == FF_B_TYPE || s->low_delay) {
            pdecpic= (AVFrame*)s->current_picture_ptr;
        } else if (s->last_picture_ptr != NULL) {
            pdecpic= (AVFrame*)s->last_picture_ptr;
        }
        //AVFrame* pdecpic=(AVFrame*)s->current_picture_ptr;
        if(pdecpic && pdecpic->data[0])
        {
                int ret=0;
                //log_openfile_with_num(log_fd_frame_data,"frame_data_Y_nv12",(int)pdecpic->opaque);
        ...
        //    av_log(NULL,AV_LOG_WARNING,"pict->data[0]=%p,pict->linesize[0]*avctx->height-16=%d.\n",pict->data[0],pict->linesize[0]*avctx->height-16);
                ...
                ptmpu-=htmp*wtmp;
                ptmpv-=htmp*wtmp;
                //log_openfile_with_num(log_fd_frame_data,"frame_data_U_nv12",pdecpic->opaque);
                log_openfile(log_fd_frame_data,"frame_data_U");
                log_dump(log_fd_frame_data,ptmpu,htmp*wtmp);
                log_closefile(log_fd_frame_data);
                //log_openfile_with_num(log_fd_frame_data,"frame_data_V_nv12",pdecpic->opaque);
        ...
        //    av_log(NULL,AV_LOG_WARNING,"end free.\n");
            ...
            if (1 || s->current_picture_ptr->pict_type==FF_B_TYPE) {
                dump_yuv_data("framedata", (void*)s->current_picture_ptr, (unsigned int)s->width, (unsigned int)s->height, 1, 0, log_cur_frame);
            } else if (s->last_picture_ptr) {
                dump_yuv_data("framedata", (void*)s->last_picture_ptr, (unsigned int)s->width, (unsigned int)s->height, 1, 0, log_cur_frame);
            }
#endif
#ifdef __dump_yuv420_nv12__
            if (1 || s->current_picture_ptr->pict_type==FF_B_TYPE) {
                dump_yuv_data("framedata", (void*)s->current_picture_ptr, (unsigned int)s->width, (unsigned int)s->height, 1, 1, log_cur_frame);
            } else if (s->last_picture_ptr) {
                dump_yuv_data("framedata", (void*)s->last_picture_ptr, (unsigned int)s->width, (unsigned int)s->height, 1, 1, log_cur_frame);
            }
#endif
#ifdef __dump_yuv420_mb__
            if (1 || s->current_picture_ptr->pict_type==FF_B_TYPE) {
                dump_yuv_data_mb("framemb", (void*)s->current_picture_ptr, (unsigned int)s->width, (unsigned int)s->height, 1, log_cur_frame);
            } else if (s->last_picture_ptr) {
                dump_yuv_data_mb("framemb", (void*)s->last_picture_ptr, (unsigned int)s->width, (unsigned int)s->height, 1, log_cur_frame);
            }
#endif
...
#ifdef __dump_deblock_DSP_TXT__
log_closefile(log_fd_dsp_deblock_text);
#endif
...
av_cold int ff_rv34_nv12_decode_end(AVCodecContext *avctx)
{
#ifdef __log_dump_decocer_input_data__
