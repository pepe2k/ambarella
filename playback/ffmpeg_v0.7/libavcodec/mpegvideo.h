...
#define HAS_CBP(a)        ((a)&MB_TYPE_CBP)
    ...
    int ref_added_for_extern; //ref count added, for extern component, for direct rendering ref count issue
} Picture;
...
/* the following codec id fields are deprecated in favor of codec_id */
    ...
    ScanTable* p_intra_scantable;
    ScanTable* p_intra_h_scantable;
    ScanTable* p_intra_v_scantable;
    ScanTable* p_inter_scantable;
    ScanTable amba_intra_scantable;
    ScanTable amba_intra_h_scantable;
    ScanTable amba_intra_v_scantable;
    ScanTable amba_inter_scantable; ///< if inter == intra then intra should be used to reduce tha cache usage
#endif
...
#ifdef __tmp_use_amba_permutation__
    uint16_t* p_intra_matrix;
    uint16_t* p_chroma_intra_matrix;
    uint16_t* p_inter_matrix;
    uint16_t* p_chroma_inter_matrix;
    
    uint16_t amba_intra_matrix[64];
    uint16_t amba_chroma_intra_matrix[64];
    uint16_t amba_inter_matrix[64];
    uint16_t amba_chroma_inter_matrix[64];
#endif
...
//FIXME some prefix?
    ...
    int scalability;
    int scalability_real;            ///< to save real scalability value
    int hierachy_type;
...
#define SLICE_NOEND     -3 ///<no end marker or error found but mb count exceeded
    ...
    DCTELEM logdct[6][64];
    DCTELEM logdct_acdc[6][64];
    DCTELEM logdct_deqaun[6][64];
    char        logdct_result[6][64];
    char        logdctmc_result[6][64];
    DCTELEM logdct_tmp[64];
    #endif
} MpegEncContext;
...
//nv12 is special
void ff_mspel_motion_nv12(MpegEncContext *s,
                               uint8_t *dest_y, uint8_t *dest_cb,
                               uint8_t **ref_picture, op_pixels_func (*pix_op)[4],op_pixels_func (*pix_op_nv12)[4],
                               int motion_x, int motion_y, int h);
void MPV_frame_end_nv12(MpegEncContext *s);
void ff_init_block_index_nv12(MpegEncContext *s);
void ff_init_block_index_amba(MpegEncContext *s);
extern const enum PixelFormat ff_pixfmt_list_nv12[];

int16_t *h263_pred_motion_amba(MpegEncContext * s, int block, int dir,
                        int *px, int *py);
void h263_decode_init_vlc_nv12(MpegEncContext *s);
void h263_decode_init_vlc_amba(MpegEncContext *s);
int ff_mpeg4_decode_picture_header_nv12(MpegEncContext * s, GetBitContext *gb);
int ff_h263_decode_mb_nv12(MpegEncContext *s,
                      DCTELEM block[6][64]);
int ff_h263_decode_mb_amba(MpegEncContext *s,
                      DCTELEM block[6][64]);
int ff_mpeg4_decode_mb_nv12(MpegEncContext *s,
                      DCTELEM block[6][64]);
int ff_mpeg4_decode_mb_amba(MpegEncContext *s,
                      DCTELEM block[6][64]);
void ff_mpeg4_clean_buffers_nv12(MpegEncContext *s);
int ff_h263_resync_nv12(MpegEncContext *s);

static inline void ff_update_block_index_nv12(MpegEncContext *s);
static inline void ff_update_block_index_amba(MpegEncContext *s);
...
static inline void ff_update_block_index_nv12(MpegEncContext *s)
{
    const int block_size= 16>>s->avctx->lowres;
