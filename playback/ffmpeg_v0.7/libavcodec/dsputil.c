...
void ff_put_pixels_clamped_nv12_c(const DCTELEM *block, uint8_t *restrict pixels,
                                 int line_size)
{
    int i;
    uint8_t *cm = ff_cropTbl + MAX_NEG_CROP;
    ...
    for(i=0;i<8;i++) {
        ...
        pixels[6] = cm[block[3]];
        pixels[8] = cm[block[4]];
        pixels[10] = cm[block[5]];
        pixels[12] = cm[block[6]];
        pixels[14] = cm[block[7]];
...
void ff_add_pixels_clamped_nv12_c(const DCTELEM *block, uint8_t *restrict pixels,
                                  int line_size)
{
    int i;
    uint8_t *cm = ff_cropTbl + MAX_NEG_CROP;
    ...
    for(i=0;i<8;i++) {
        pixels[0] = cm[pixels[0] + block[0]];
        pixels[2] = cm[pixels[2] + block[1]];
        pixels[4] = cm[pixels[4] + block[2]];
        pixels[6] = cm[pixels[6] + block[3]];
        pixels[8] = cm[pixels[8] + block[4]];
        pixels[10] = cm[pixels[10] + block[5]];
        pixels[12] = cm[pixels[12] + block[6]];
        pixels[14] = cm[pixels[14] + block[7]];
        pixels += line_size;
        block += 8;
    }
}
...
static void gmc1_c(uint8_t *dst, uint8_t *src, int stride, int h, int x16, int y16, int rounder)
{
    ...
    //av_log(NULL,AV_LOG_ERROR,"****!!! in gmc1 gmc1.\n");
    //av_log(NULL,AV_LOG_ERROR,"emm.\n");

    #ifdef __dump_temp__
    static int fileopen=0;
    if(!fileopen)
    {
        log_openfile_f(log_fd_temp,"gmcdebug");
        log_text_f(log_fd_temp,"check start:\n");
    }
    fileopen++;
    log_text_f(log_fd_temp,"in gmc1_c, ref:\n");
    log_text_rect_char_hex_f(log_fd_temp,src,9,16,stride-9);
    log_text_f(log_fd_temp,"before gmc1:\n");
    log_text_rect_char_hex_f(log_fd_temp,dst,8,16,stride-8);
    #endif
    ...
    #ifdef __dump_temp__
    log_text_f(log_fd_temp,"after gmc1:\n");
    log_text_rect_char_hex_f(log_fd_temp,dst,8,16,stride-8);
    if(fileopen>100)
    {
...
static void gmc1_nv12_c(uint8_t *dst, uint8_t *src, int stride, int h, int x16, int y16, int rounder)
{
    const int A=(16-x16)*(16-y16);
    const int B=(   x16)*(16-y16);
    const int C=(16-x16)*(   y16);
    ...
    //av_log(NULL,AV_LOG_ERROR,"****!!! in gmc1 nv12.\n");
    //av_log(NULL,AV_LOG_ERROR,"emm.\n");

    for(i=0; i<h; i++)
    {
        dst[0]= (A*src[0] + B*src[2] + C*src[stride+0] + D*src[stride+2] + rounder)>>8;
        dst[2]= (A*src[2] + B*src[4] + C*src[stride+2] + D*src[stride+4] + rounder)>>8;
        dst[4]= (A*src[4] + B*src[6] + C*src[stride+4] + D*src[stride+6] + rounder)>>8;
        dst[6]= (A*src[6] + B*src[8] + C*src[stride+6] + D*src[stride+8] + rounder)>>8;
        dst[8]= (A*src[8] + B*src[10] + C*src[stride+8] + D*src[stride+10] + rounder)>>8;
        dst[10]= (A*src[10] + B*src[12] + C*src[stride+10] + D*src[stride+12] + rounder)>>8;
        dst[12]= (A*src[12] + B*src[14] + C*src[stride+12] + D*src[stride+14] + rounder)>>8;
        dst[14]= (A*src[14] + B*src[16] + C*src[stride+14] + D*src[stride+16] + rounder)>>8;
        dst+= stride;
        src+= stride;
    }
}
                  ...
                  int dxx, int dxy, int dyx, int dyy, int shift, int r, int width, int height)
{
    ...
    //av_log(NULL,AV_LOG_ERROR,"****!!! in gmc gmc.\n");
    //av_log(NULL,AV_LOG_ERROR,"emm.\n");
...
void ff_gmc_nv12_c(uint8_t *dst, uint8_t *src, int stride, int h, int ox, int oy,
                  int dxx, int dxy, int dyx, int dyy, int shift, int r, int width, int height)
{
    int y, vx, vy;
    const int s= 1<<shift;

    //av_log(NULL,AV_LOG_ERROR,"****!!! in gmc nv12.\n");
    //av_log(NULL,AV_LOG_ERROR,"emm.\n");
    ...
    for(y=0; y<h; y++){
        ...
        vx= ox;
        vy= oy;
        for(x=0; x<16; x+=2){ //XXX FIXME optimize
            int src_x, src_y, frac_x, frac_y, index;

            //should be error here, >>16?
            src_x= vx>>16;
            src_y= vy>>16;
            frac_x= src_x&(s-1);
            frac_y= src_y&(s-1);
            ...
            if((unsigned)src_x < width){
                if((unsigned)src_y < height){
                    index= src_x*2 + src_y*stride;
                    dst[y*stride + x]= (  (  src[index         ]*(s-frac_x)
                                           + src[index       +2]*   frac_x )*(s-frac_y)
                                        + (  src[index+stride  ]*(s-frac_x)
                                           + src[index+stride+2]*   frac_x )*   frac_y
                                        + r)>>(shift*2);
                }else{
                    index= src_x*2 + av_clip(src_y, 0, height)*stride;
                    dst[y*stride + x]= ( (  src[index         ]*(s-frac_x)
                                          + src[index       +2]*   frac_x )*s
                                        + r)>>(shift*2);
                }
            }else{
                if((unsigned)src_y < height){
                    index= (av_clip(src_x, 0, width)*2) + src_y*stride;
                    dst[y*stride + x]= (  (  src[index         ]*(s-frac_y)
                                           + src[index+stride  ]*   frac_y )*s
                                        + r)>>(shift*2);
                }else{
                    index= (av_clip(src_x, 0, width)*2) + av_clip(src_y, 0, height)*stride;
                    dst[y*stride + x]=    src[index         ];
                }
            }
...
static void h263_h_loop_filter_nv12_c(uint8_t *src, int stride, int qscale){
    if(CONFIG_H263_DECODER || CONFIG_H263_ENCODER) {
    int y;
    const int strength= ff_h263_loop_filter_strength[qscale];
    ...
    for(y=0; y<8; y++){
        int d1, d2, ad1;
        int p0= src[y*stride-4];
        int p1= src[y*stride-2];
        int p2= src[y*stride+0];
        int p3= src[y*stride+2];
        int d = (p0 - p3 + 4*(p2 - p1)) / 8;
        ...
        src[y*stride-4] = p0 - d2;
        src[y*stride+2] = p3 + d2;
    }
    }
}
...
//    av_log(NULL,AV_LOG_ERROR,"log in dsputil_init: avctx->dct_algo=%d,avctx->lowres=%d.\n",avctx->dct_algo,avctx->lowres);
    ...
    if(avctx->lowres==1){
        ...
        if(avctx->idct_algo==FF_IDCT_INT){
            ...
            c->idct_put_nv12= ff_simple_idct_put_nv12;
            c->idct_add_nv12= ff_simple_idct_add_nv12;
            c->idct    = ff_simple_idct;
            c->idct_permutation_type= FF_NO_IDCT_PERM;
        }
    }

//    av_log(NULL,AV_LOG_ERROR,"log in dsputil_init: c->idct_permutation_type=%d.\n",c->idct_permutation_type);

    c->get_pixels = get_pixels_c;
    c->diff_pixels = diff_pixels_c;
    c->put_pixels_clamped = ff_put_pixels_clamped_c;
    c->put_pixels_clamped_chroma_nv12 = ff_put_pixels_clamped_nv12_c;
    c->put_signed_pixels_clamped = ff_put_signed_pixels_clamped_c;
    c->put_pixels_nonclamped = put_pixels_nonclamped_c;
    c->add_pixels_clamped = ff_add_pixels_clamped_c;
    c->add_pixels_clamped_chroma_nv12 = ff_add_pixels_clamped_nv12_c;
    c->sum_abs_dctelem = sum_abs_dctelem_c;
    c->gmc1 = gmc1_c;
    c->gmc = ff_gmc_c;
    c->gmc1_nv12 = gmc1_nv12_c;
    c->gmc_nv12 = ff_gmc_nv12_c;
    c->pix_sum = pix_sum_c;
...
#define SET_CMP_FUNC(name) \
    ...
    if (CONFIG_H263_DECODER || CONFIG_H263_ENCODER) {
        ...
        c->h263_h_loop_filter_nv12= h263_h_loop_filter_nv12_c;
    }
...
#define dspfunc1nv12(PFX, IDX, NUM, depth)\
    c->PFX ## _pixels_tab_nv12[IDX][0] = FUNCC(PFX ## _pixels ## NUM ## _nv12,      depth);\
    c->PFX ## _pixels_tab_nv12[IDX][1] = FUNCC(PFX ## _pixels ## NUM ## _nv12_x2,   depth);\
    c->PFX ## _pixels_tab_nv12[IDX][2] = FUNCC(PFX ## _pixels ## NUM ## _nv12_y2,   depth);\
    c->PFX ## _pixels_tab_nv12[IDX][3] = FUNCC(PFX ## _pixels ## NUM ## _nv12_xy2,  depth);\
...
\
    c->draw_edges_nv12               = FUNCC(draw_edges_nv12          , depth);\
    c->emulated_edge_mc_nv12         = FUNC (ff_emulated_edge_mc_nv12 , depth);\
\
...
\
    c->put_h264_chroma_pixels_tab_nv12[0]= FUNCC(put_h264_chroma_mc8_nv12   , depth);\
    c->put_h264_chroma_pixels_tab_nv12[1]= FUNCC(put_h264_chroma_mc4_nv12   , depth);\
    c->put_h264_chroma_pixels_tab_nv12[2]= FUNCC(put_h264_chroma_mc2_nv12   , depth);\
    c->avg_h264_chroma_pixels_tab_nv12[0]= FUNCC(avg_h264_chroma_mc8_nv12   , depth);\
    c->avg_h264_chroma_pixels_tab_nv12[1]= FUNCC(avg_h264_chroma_mc4_nv12   , depth);\
    c->avg_h264_chroma_pixels_tab_nv12[2]= FUNCC(avg_h264_chroma_mc2_nv12   , depth);\
\
...
\
    dspfunc1nv12(put         , 1, 8, depth);\
    dspfunc1nv12(put_no_rnd  , 1, 8, depth);\
    dspfunc1nv12(avg         , 1, 8, depth);\
    dspfunc1nv12(avg_no_rnd  , 1, 8, depth);\
\
    ...
    /*record ffmpeg orignal idct*/
    c->idct_put_original = c->idct_put;
    c->idct_add_original = c->idct_add;
    c->idct_put_nv12_original = c->idct_put_nv12;
    c->idct_add_nv12_original = c->idct_add_nv12;
    c->idct_original = c->idct;

    #ifdef __use_reference_dct__
        Initialize_Reference_IDCT();
        c->idct_put= Reference_IDCT_put;
        c->idct_add= Reference_IDCT_add;
        c->idct_put_nv12= Reference_IDCT_put_nv12;
        c->idct_add_nv12= Reference_IDCT_add_nv12;
        c->idct    = Reference_IDCT;
        c->idct_permutation_type= FF_NO_IDCT_PERM;
    #endif
    ...
    if ((CONFIG_VP3_DECODER || CONFIG_VP5_DECODER || CONFIG_VP6_DECODER ) &&
                avctx->idct_algo==FF_IDCT_VP3) {
        av_log(NULL,AV_LOG_DEBUG,"vpX, no override idct function.\n");
    }else {
        c->idct_put= CM_IDCT_put;//diff
        c->idct_add= CM_IDCT_add;//diff
        av_log(NULL,AV_LOG_DEBUG,"no vpX, override idct function.\n");
    }

    c->idct    = CM_IDCT;//same
    c->idct_put_nv12= CM_IDCT_put_nv12;
    c->idct_add_nv12= CM_IDCT_add_nv12;
    c->idct_permutation_type= FF_NO_IDCT_PERM;
    #endif

    //add amba dsp support
    #ifdef __tmp_use_amba_permutation__
    c->amba_idct_permutation_type= FF_TRANSPOSE_IDCT_PERM;
    for(i=0; i<64; i++)
        c->amba_idct_permutation[i]= ((i&7)<<3) | (i>>3);
    #endif
...
//        av_log(NULL,AV_LOG_ERROR,"****log in dsputil_init: c->idct_permutation_type=FF_NO_IDCT_PERM.\n");
        break;
