...
\
static void OPNAME ## rv40_chroma_mc4_c_nv12(uint8_t *dst/*align 8*/, uint8_t *src/*align 1*/, int stride, int h, int x, int y){\
    const int A=(8-x)*(8-y);\
    const int B=(  x)*(8-y);\
    const int C=(8-x)*(  y);\
    const int D=(  x)*(  y);\
...
\
    if(D){\
        for(i=0; i<h; i++){\
            OP(dst[0], (A*src[0] + B*src[2] + C*src[stride+0] + D*src[stride+2] + bias));\
            OP(dst[2], (A*src[2] + B*src[4] + C*src[stride+2] + D*src[stride+4] + bias));\
            OP(dst[4], (A*src[4] + B*src[6] + C*src[stride+4] + D*src[stride+6] + bias));\
            OP(dst[6], (A*src[6] + B*src[8] + C*src[stride+6] + D*src[stride+8] + bias));\
            dst+= stride;\
            src+= stride;\
        }\
    }else{\
        const int E= B+C;\
        const int step= C ? stride : 2;\
        for(i=0; i<h; i++){\
            OP(dst[0], (A*src[0] + E*src[step+0] + bias));\
            OP(dst[2], (A*src[2] + E*src[step+2] + bias));\
            OP(dst[4], (A*src[4] + E*src[step+4] + bias));\
...
\
static void OPNAME ## rv40_chroma_mc8_c_nv12(uint8_t *dst/*align 8*/, uint8_t *src/*align 1*/, int stride, int h, int x, int y){\
    const int A=(8-x)*(8-y);\
    const int B=(  x)*(8-y);\
    const int C=(8-x)*(  y);\
    const int D=(  x)*(  y);\
...
\
    if(D){\
        for(i=0; i<h; i++){\
            ...
            OP(dst[4], (A*src[4] + B*src[6] + C*src[stride+4] + D*src[stride+6] + bias));\
            OP(dst[6], (A*src[6] + B*src[8] + C*src[stride+6] + D*src[stride+8] + bias));\
            OP(dst[8], (A*src[8] + B*src[10] + C*src[stride+8] + D*src[stride+10] + bias));\
            OP(dst[10], (A*src[10] + B*src[12] + C*src[stride+10] + D*src[stride+12] + bias));\
            OP(dst[12], (A*src[12] + B*src[14] + C*src[stride+12] + D*src[stride+14] + bias));\
            OP(dst[14], (A*src[14] + B*src[16] + C*src[stride+14] + D*src[stride+16] + bias));\
            dst+= stride;\
            src+= stride;\
        }\
    }else{\
        ...
        for(i=0; i<h; i++){\
            ...
            OP(dst[4], (A*src[4] + E*src[step+4] + bias));\
            OP(dst[6], (A*src[6] + E*src[step+6] + bias));\
            OP(dst[8], (A*src[8] + E*src[step+8] + bias));\
            OP(dst[10], (A*src[10] + E*src[step+10] + bias));\
            OP(dst[12], (A*src[12] + E*src[step+12] + bias));\
            OP(dst[14], (A*src[14] + E*src[step+14] + bias));\
            dst+= stride;\
            src+= stride;\
        }\
    }\
...
void ff_rv40dsp_init(DSPContext* c, AVCodecContext *avctx) {
    ...
    //nv12
    c->put_rv40_chroma_pixels_tab_nv12[0]= put_rv40_chroma_mc8_c_nv12;
    c->put_rv40_chroma_pixels_tab_nv12[1]= put_rv40_chroma_mc4_c_nv12;
    c->avg_rv40_chroma_pixels_tab_nv12[0]= avg_rv40_chroma_mc8_c_nv12;
    c->avg_rv40_chroma_pixels_tab_nv12[1]= avg_rv40_chroma_mc4_c_nv12;
}
