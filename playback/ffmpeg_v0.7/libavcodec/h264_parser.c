                      ...
                      const uint8_t *buf, int buf_size)
{
    ...
    if (!h->got_first) {
        ...
        if (avctx->extradata_size) {
            //av_log(NULL, AV_LOG_ERROR, "@@@@@@h264_parse: invoke ff_h264_decode_extradata\n");
            h->s.avctx = avctx;
    ...
    // update the frame size from h264 sps, chuchen_2011_11_23
    // support interlaced case and adjust width and height
    tmp_width = h->sps.mb_width;
    tmp_height = h->sps.mb_height * (2 - h->sps.frame_mbs_only_flag);

    avctx->width = 16*tmp_width - 2*FFMIN(h->sps.crop_right, 7);
    if(h->sps.frame_mbs_only_flag)
        avctx->height = 16*tmp_height - 2*FFMIN(h->sps.crop_bottom, 7);
    else
        avctx->height = 16*tmp_height - 4*FFMIN(h->sps.crop_bottom, 7);
