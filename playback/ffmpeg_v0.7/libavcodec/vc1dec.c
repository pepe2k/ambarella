...
//                if(!coded_inter) coded_inter = !is_intra[i] & is_coded[i];
                if(!coded_inter) coded_inter = (!is_intra[i]) & (is_coded[i]);//NOTE: parenthese
            }
...
//    if (effect_type = get_bits_long(gb, 30)){
    if ((effect_type = get_bits_long(gb, 30))){//NOTE: parentheses
...
//    if (effect_flag = get_bits1(gb))
    if ((effect_flag = get_bits1(gb)))//NOTE: parentheses
...
static av_cold int vc1_decode_init(AVCodecContext *avctx)
{
    ...
    /*restore idct to ffmpeg orignal*/
    s->dsp.idct_put = s->dsp.idct_put_original;
    s->dsp.idct_add = s->dsp.idct_add_original;
    s->dsp.idct_put_nv12 = s->dsp.idct_put_nv12_original;
    s->dsp.idct_add_nv12 = s->dsp.idct_add_nv12_original;
    s->dsp.idct =  s->dsp.idct_original;
