...
//av_cold int ff_msmpeg4_decode_init_nv12(MpegEncContext *s);
static inline void set_intra_p(mb_mv_P_t* pmvp);
int wmv2_decode_block_inter_dequant(MpegEncContext * s, DCTELEM * block, int n, int coded, const uint8_t *scan_table);
static inline void dct_unquantize_mpeg2_intra(MpegEncContext *s,
                                   DCTELEM *block, int n, int qscale);
static void inline dct_unquantize_h263_intra(MpegEncContext *s,
                                  DCTELEM *block, int n, int qscale);
int ff_msmpeg4_decode_block_amba_intra_dequant(MpegEncContext * s, DCTELEM * block,
                              int n, int coded);
int ff_msmpeg4_decode_block_amba_inter_dequant(MpegEncContext * s, DCTELEM * block,
                              int n, int coded);
static int msmpeg4v12_decode_mb_amba(MpegEncContext *s, DCTELEM block[6][64]);
static int msmpeg4v34_decode_mb_amba(MpegEncContext *s, DCTELEM block[6][64]);
...
static av_cold void common_init(MpegEncContext * s)
{
    ...
    if(s->msmpeg4_version>=4){
        ...
        ff_init_scantable(s->dsp.amba_idct_permutation, &s->amba_intra_scantable  , wmv1_scantable[1]);
        ff_init_scantable(s->dsp.amba_idct_permutation, &s->amba_intra_h_scantable, wmv1_scantable[2]);
        ff_init_scantable(s->dsp.amba_idct_permutation, &s->amba_intra_v_scantable, wmv1_scantable[3]);
        ff_init_scantable(s->dsp.amba_idct_permutation, &s->amba_inter_scantable  , wmv1_scantable[0]);
#endif
    }
...
static int msmpeg4v12_decode_mb(MpegEncContext *s, DCTELEM block[6][64])
{
    ...
    if (!s->mb_intra) {
        ...
        h263_pred_motion_amba(s, 0, 0, &mx, &my);
...
av_cold int ff_msmpeg4_decode_init(AVCodecContext *avctx)
{
    ...
    if(s->codec_id==CODEC_ID_AMBA_P_MSMPEG4V1 || s->codec_id==CODEC_ID_AMBA_P_MSMPEG4V2 ||s->codec_id==CODEC_ID_AMBA_P_MSMPEG4V3 || s->codec_id==CODEC_ID_AMBA_P_WMV1 || s->codec_id==CODEC_ID_AMBA_P_WMV2)
    {
        switch(s->msmpeg4_version){
        case 1:
        case 2:
            s->decode_mb= msmpeg4v12_decode_mb_amba;
            break;
        case 3:
        case 4:
            s->decode_mb= msmpeg4v34_decode_mb_amba;
            break;
        case 5:
...
int msmpeg4_decode_ext_header(MpegEncContext * s, int buf_size)
{
    ...
    if(left>=length && left<length+8)
    {
        ...
        av_log(NULL, AV_LOG_DEBUG, "fps:%2d bps:%2d roundingType:%1d\n", fps, s->bit_rate/1024, s->flipflop_rounding);
    }
...
av_cold int ff_msmpeg4_decode_init_nv12(MpegEncContext *s)
{
    ...
    if (!done) {
        ...
        INIT_VLC_STATIC(&ff_h263_inter_MCBPC_vlc, INTER_MCBPC_VLC_BITS, 25,
...
int wmv2_decode_block_inter_dequant(MpegEncContext * s, DCTELEM * block, int n, int coded, const uint8_t *scan_table)
{
    ...
    ambadec_assert_ffmpeg(scan_table);
    {
        qmul = s->qscale << 1;
        qadd = (s->qscale - 1) | 1;
        i = -1;
...
//printf("level length:%d, run length: %d\n", ll, s->esc3_run_length);
...
#endif
                    GET_RL_VLC(level, run, re, &s->gb, rl_vlc, TEX_VLC_BITS, 2, 1);
                    i+= run + rl->max_run[run>>7][level] + run_diff; //FIXME opt indexing
                    level = (level ^ SHOW_SBITS(re, &s->gb, 1)) - SHOW_SBITS(re, &s->gb, 1);
                    LAST_SKIP_BITS(re, &s->gb, 1);
#ifdef ERROR_DETAILS
                if(run==66)
        ...
        if (i > 62){
            ...
            if(i&(~63)){
                const int left= s->gb.size_in_bits - get_bits_count(&s->gb);
                if(((i+192 == 64 && level==-1) || s->error_recognition<=1) && left>=0){
                    av_log(s->avctx, AV_LOG_ERROR, "ignoring overflow at %d %d\n", s->mb_x, s->mb_y);
                    break;
                }else{
                    av_log(s->avctx, AV_LOG_ERROR, "ac-tex damaged at %d %d\n", s->mb_x, s->mb_y);
                              ...
                              int n, int coded)
{
    ...
    #ifdef __tmp_use_amba_permutation__
    scan_table = s->p_inter_scantable->permutated;
    #else
    scan_table = s->inter_scantable.permutated;
    #endif
    rl_vlc= rl->rl_vlc[s->qscale];
...
//    pmvp->mb_setting.frame_top=!s->mb_y;
    ...
    if (!s->mb_intra) {
        ...
        for (i = 0; i < 6; i++) {
            if (ff_msmpeg4_decode_block_amba_inter_dequant(s, block[i], i, (cbp >> (5 - i)) & 1) < 0)
            {
                av_log(s->avctx, AV_LOG_ERROR, "\nerror while decoding block: %d x %d (%d)\n", s->mb_x, s->mb_y, i);
                return -1;
            }
...
static int msmpeg4v34_decode_mb_amba(MpegEncContext *s, DCTELEM block[6][64])
{
    int cbp, code, i;
    uint8_t *coded_val;
    uint32_t * const mb_type_ptr= &s->current_picture_ptr->mb_type[ s->mb_x + s->mb_y*s->mb_stride ];
    H263AmbaDecContext_t* thiz=(H263AmbaDecContext_t*)s;
    h263_pic_swinfo_t* pinfo=thiz->p_pic->pswinfo+s->mb_x + s->mb_y*s->mb_width;
//    int* pcpdes, pcpval;
    assert(s->h263_pred);
