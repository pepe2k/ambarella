...
int ff_mpa_decode_header(AVCodecContext *avctx, uint32_t head, int *sample_rate, int *channels, int *frame_size, int *bit_rate)
{
    ...
    case 3:
        ...
        if(s->sample_rate == 8000){
            avctx->codec_id = CODEC_ID_MP3PV;
	    }
        if (s->lsf)
            *frame_size = 576;
        else
    ...
    for(i=0; i<buf_size; ){
        if(s->frame_size){
            ...
            while(i<buf_size){
                ...
                if (ret < 4) {
                    //in order to read correct bit_rate
                    //update to ffmpeg-0.10,chuchen 2012_3_20
                    if(i > 4)
                        s->header_count= -2;
                } else {
...
AVCodecParser ff_mpegaudio_parser = {
    { CODEC_ID_MP1, CODEC_ID_MP2, CODEC_ID_MP3, CODEC_ID_MP3PV },
