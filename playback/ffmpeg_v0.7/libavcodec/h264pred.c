...
\
    if(codec_id != CODEC_ID_RV40){\
        h->pred4x4_nv12[VERT_PRED           ]= FUNCC(pred4x4_vertical_nv12                  , depth);\
        h->pred4x4_nv12[HOR_PRED            ]= FUNCC(pred4x4_horizontal_nv12                , depth);\
        h->pred4x4_nv12[DC_PRED             ]= FUNCC(pred4x4_dc_nv12                        , depth);\
        if(codec_id == CODEC_ID_SVQ3)\
            h->pred4x4_nv12[DIAG_DOWN_LEFT_PRED ]= FUNCC(pred4x4_down_left_svq3_nv12        , depth);\
        else\
            h->pred4x4_nv12[DIAG_DOWN_LEFT_PRED ]= FUNCC(pred4x4_down_left_nv12             , depth);\
        h->pred4x4_nv12[DIAG_DOWN_RIGHT_PRED]= FUNCC(pred4x4_down_right_nv12                , depth);\
        h->pred4x4_nv12[VERT_RIGHT_PRED     ]= FUNCC(pred4x4_vertical_right_nv12            , depth);\
        h->pred4x4_nv12[HOR_DOWN_PRED       ]= FUNCC(pred4x4_horizontal_down_nv12           , depth);\
        h->pred4x4_nv12[VERT_LEFT_PRED      ]= FUNCC(pred4x4_vertical_left_nv12             , depth);\
        h->pred4x4_nv12[HOR_UP_PRED         ]= FUNCC(pred4x4_horizontal_up_nv12             , depth);\
        h->pred4x4_nv12[LEFT_DC_PRED        ]= FUNCC(pred4x4_left_dc_nv12                   , depth);\
        h->pred4x4_nv12[TOP_DC_PRED         ]= FUNCC(pred4x4_top_dc_nv12                    , depth);\
        h->pred4x4_nv12[DC_128_PRED         ]= FUNCC(pred4x4_128_dc_nv12                    , depth);\
    }else{\
        h->pred4x4_nv12[VERT_PRED           ]= FUNCC(pred4x4_vertical_nv12                  , depth);\
        h->pred4x4_nv12[HOR_PRED            ]= FUNCC(pred4x4_horizontal_nv12                , depth);\
        h->pred4x4_nv12[DC_PRED             ]= FUNCC(pred4x4_dc_nv12                        , depth);\
        h->pred4x4_nv12[DIAG_DOWN_LEFT_PRED ]= FUNCC(pred4x4_down_left_rv40_nv12            , depth);\
        h->pred4x4_nv12[DIAG_DOWN_RIGHT_PRED]= FUNCC(pred4x4_down_right_nv12                , depth);\
        h->pred4x4_nv12[VERT_RIGHT_PRED     ]= FUNCC(pred4x4_vertical_right_nv12            , depth);\
        h->pred4x4_nv12[HOR_DOWN_PRED       ]= FUNCC(pred4x4_horizontal_down_nv12           , depth);\
        h->pred4x4_nv12[VERT_LEFT_PRED      ]= FUNCC(pred4x4_vertical_left_rv40_nv12        , depth);\
        h->pred4x4_nv12[HOR_UP_PRED         ]= FUNCC(pred4x4_horizontal_up_rv40_nv12        , depth);\
        h->pred4x4_nv12[LEFT_DC_PRED        ]= FUNCC(pred4x4_left_dc_nv12                   , depth);\
        h->pred4x4_nv12[TOP_DC_PRED         ]= FUNCC(pred4x4_top_dc_nv12                    , depth);\
        h->pred4x4_nv12[DC_128_PRED         ]= FUNCC(pred4x4_128_dc_nv12                    , depth);\
        h->pred4x4_nv12[DIAG_DOWN_LEFT_PRED_RV40_NODOWN]= FUNCC(pred4x4_down_left_rv40_nodown_nv12  , depth);\
        h->pred4x4_nv12[HOR_UP_PRED_RV40_NODOWN]= FUNCC(pred4x4_horizontal_up_rv40_nodown_nv12      , depth);\
        h->pred4x4_nv12[VERT_LEFT_PRED_RV40_NODOWN]= FUNCC(pred4x4_vertical_left_rv40_nodown_nv12   , depth);\
    }\
\
...
\
    h->pred8x8_nv12[VERT_PRED8x8   ]= FUNCC(pred8x8_vertical_nv12                         , depth);\
    h->pred8x8_nv12[HOR_PRED8x8    ]= FUNCC(pred8x8_horizontal_nv12                       , depth);\
    h->pred8x8_nv12[PLANE_PRED8x8  ]= FUNCC(pred8x8_plane_nv12                            , depth);\
    if(codec_id != CODEC_ID_RV40 ){\
        h->pred8x8_nv12[DC_PRED8x8     ]= FUNCC(pred8x8_dc_nv12                           , depth);\
        h->pred8x8_nv12[LEFT_DC_PRED8x8]= FUNCC(pred8x8_left_dc_nv12                      , depth);\
        h->pred8x8_nv12[TOP_DC_PRED8x8 ]= FUNCC(pred8x8_top_dc_nv12                       , depth);\
        h->pred8x8_nv12[ALZHEIMER_DC_L0T_PRED8x8 ]= FUNC(pred8x8_mad_cow_dc_l0t_nv12      , depth);\
        h->pred8x8_nv12[ALZHEIMER_DC_0LT_PRED8x8 ]= FUNC(pred8x8_mad_cow_dc_0lt_nv12      , depth);\
        h->pred8x8_nv12[ALZHEIMER_DC_L00_PRED8x8 ]= FUNC(pred8x8_mad_cow_dc_l00_nv12      , depth);\
        h->pred8x8_nv12[ALZHEIMER_DC_0L0_PRED8x8 ]= FUNC(pred8x8_mad_cow_dc_0l0_nv12      , depth);\
    }else{\
        h->pred8x8_nv12[DC_PRED8x8     ]= FUNCC(pred8x8_dc_rv40_nv12                       , depth);\
        h->pred8x8_nv12[LEFT_DC_PRED8x8]= FUNCC(pred8x8_left_dc_rv40_nv12                  , depth);\
        h->pred8x8_nv12[TOP_DC_PRED8x8 ]= FUNCC(pred8x8_top_dc_rv40_nv12                   , depth);\
    }\
    h->pred8x8_nv12[DC_128_PRED8x8 ]= FUNCC(pred8x8_128_dc_nv12                            , depth);\
\
