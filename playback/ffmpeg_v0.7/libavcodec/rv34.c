                          ...
                          h264_chroma_mc_func (*chroma_mc))
{
    ...
    if (block_type == RV34_MB_P_16x8)
    {
        ...
        log_text_rect_char_hex(log_fd_dsp_text,srcU,8,4,s->uvlinesize-8);
        #endif
        ...
        log_text_rect_char_hex(log_fd_dsp_text,U,8,4,s->uvlinesize-8);
        #endif

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<P_8x4 V: reference:>>");
        log_text_rect_char_hex(log_fd_dsp_text,srcV,8,4,s->uvlinesize-8);
        #endif
        ...
        log_text_rect_char_hex(log_fd_dsp_text,V,8,4,s->uvlinesize-8);
        #endif
    ...
    else if(block_type == RV34_MB_P_8x16)
    {
        ...
        log_text_rect_char_hex(log_fd_dsp_text,srcU,4,8,s->uvlinesize-4);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<MC U: result:>>");
        log_text_rect_char_hex(log_fd_dsp_text,U,4,8,s->uvlinesize-4);
        #endif

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<P_4x8 V: reference:>>");
        log_text_rect_char_hex(log_fd_dsp_text,srcV,4,8,s->uvlinesize-4);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<MC V: result:>>");
        log_text_rect_char_hex(log_fd_dsp_text,V,4,8,s->uvlinesize-4);
        #endif
    }
    else if(block_type == RV34_MB_P_8x8)
    {
        ...
        log_text_rect_char_hex(log_fd_dsp_text,srcU,4,4,s->uvlinesize-4);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<MC U: result:>>");
        log_text_rect_char_hex(log_fd_dsp_text,U,4,4,s->uvlinesize-4);
        #endif

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<P_4x4 V: reference:>>");
        log_text_rect_char_hex(log_fd_dsp_text,srcV,4,4,s->uvlinesize-4);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<MC V: result:>>");
        log_text_rect_char_hex(log_fd_dsp_text,V,4,4,s->uvlinesize-4);
        #endif
    }
    else
    {
        ...
        log_text_rect_char_hex(log_fd_dsp_text,srcU,8,8,s->uvlinesize-8);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<MC U: result:>>");
        log_text_rect_char_hex(log_fd_dsp_text,U,8,8,s->uvlinesize-8);
        #endif

        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<P_8x8 V: reference:>>");
        log_text_rect_char_hex(log_fd_dsp_text,srcV,8,8,s->uvlinesize-8);
        #endif
        ...
        #ifdef __dump_DSP_TXT__
        log_text(log_fd_dsp_text,"<<MC V: result:>>");
        log_text_rect_char_hex(log_fd_dsp_text,V,8,8,s->uvlinesize-8);
        #endif
    }
#else
    qpel_mc[!is16x16][dxy](Y, srcY, s->linesize);
    ...
    if(!is16){
        ...
        for(j = 0; j < 2; j++){
            ...
            for(i = 0; i < 2; i++, cbp >>= 1, idx++){
                ...
                log_text_rect_char_hex(log_fd_dsp_text,U + i*4 + j*4*s->uvlinesize,4,4,s->uvlinesize-4);
                log_text(log_fd_dsp_text,"<<after intra pred V>>");
                log_text_rect_char_hex(log_fd_dsp_text,V + i*4 + j*4*s->uvlinesize,4,4,s->uvlinesize-4);
                #endif
...
static void rv34_apply_differences(RV34DecContext *r, int cbp)
{
    ...
    int i;
        ...
        log_text_rect_char_hex(log_fd_dsp_text,s->dest[1],8,8,s->uvlinesize-8);
        log_text(log_fd_dsp_text,"<<<<< total MC result V: >>>>>");
        log_text_rect_char_hex(log_fd_dsp_text,s->dest[2],8,8,s->uvlinesize-8);
    #endif
...
static int rv34_decode_slice(RV34DecContext *r, int end, const uint8_t* buf, int buf_size)
{
    ...
    while(!check_slice_end(r, s)) {
        ...
        if (++s->mb_x == s->mb_width) {
            ...
            {
            #ifdef __log_decoding_config__
            log_dump(3,s->current_picture_ptr->data[1]+304 ,16);                        
            #endif
...
if(pdecpic && pdecpic->data[0])
{
        ...
        ptt=pdecpic->data[1];
        for(itt=0;itt<((avctx->height+1)/2);itt++,ptt+=pdecpic->linesize[1])
            log_dump(log_fd_frame_data,ptt,(avctx->width+1)/2);
        log_closefile(log_fd_frame_data);

        log_openfile(log_fd_frame_data,"frame_data_V");
        ptt=pdecpic->data[2];
        for(itt=0;itt<((avctx->height+1)/2);itt++,ptt+=pdecpic->linesize[2])
            log_dump(log_fd_frame_data,ptt,(avctx->width+1)/2);
        log_closefile(log_fd_frame_data);
}
#endif
