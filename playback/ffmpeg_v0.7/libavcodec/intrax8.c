...
static int x8_setup_spatial_predictor_amba(IntraX8Context * const w, const int chroma){
    MpegEncContext * const s= w->s;
    int range;
    int sum;
    int quant;
    ...
    s->dsp.x8_setup_spatial_compensation(s->dest[chroma], s->edge_emu_buffer,
                                          s->current_picture_ptr->linesize[chroma>0],
                                          &range, &sum, w->edges);
    if(chroma){
        w->orient=w->chroma_orient;
        quant=w->quant_dc_chroma;
...
static void x8_init_block_index_amba(MpegEncContext *s){ //FIXME maybe merge with ff_*
//not s->linesize as this would be wrong for field pics
//not that IntraX8 has interlacing support ;)
    const int linesize  = s->current_picture_ptr->linesize[0];
    const int uvlinesize= s->current_picture_ptr->linesize[1];

    s->dest[0] = s->current_picture_ptr->data[0];
    s->dest[1] = s->current_picture_ptr->data[1];
    //s->dest[2] = s->current_picture_ptr->data[2];
...
int ff_intrax8_decode_picture_amba(IntraX8Context * const w, int dquant, int quant_offset){
    MpegEncContext * const s= w->s;
    int mb_xy;
    assert(s);
    w->use_quant_matrix = get_bits1(&s->gb);
    ...
    for(s->mb_y=0; s->mb_y < s->mb_height*2; s->mb_y++){
        x8_init_block_index_amba(s);
        mb_xy=(s->mb_y>>1)*s->mb_stride;

        for(s->mb_x=0; s->mb_x < s->mb_width*2; s->mb_x++){
            x8_get_prediction(w);
            if(x8_setup_spatial_predictor_amba(w,0)) goto error;
            if(x8_decode_intra_mb(w,0)) goto error;
            ...
            if( s->mb_x & s->mb_y & 1 ){
                ...
                /*when setting up chroma, no vlc is read,
                so no error condition can be reached*/
                x8_setup_spatial_predictor_amba(w,1);
                if(x8_decode_intra_mb(w,1)) goto error;

                x8_setup_spatial_predictor_amba(w,2);
                if(x8_decode_intra_mb(w,2)) goto error;
