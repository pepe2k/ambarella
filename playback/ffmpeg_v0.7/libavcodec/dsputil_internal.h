...
//uv together, w=8
static void FUNCC(draw_edges_nv12)(uint8_t *buf, int wrap, int width, int height)
{
    uint8_t *ptr, *last_line;
    int i;
    int pixel_width=width<<1;
    uint32_t value=0;
    ...
    for(i=0;i<8;i++) {
        /* top and bottom */
        memcpy(buf - (i + 1) * wrap, buf, pixel_width);
        memcpy(last_line + (i + 1) * wrap, last_line, pixel_width);
    }
    /* left and right */
    ptr = buf;
    for(i=0;i<height;i++) {
        ...
        ptmp[0]=ptmp[2]=ptr[0];
        ptmp[1]=ptmp[3]=ptr[1];
        *((uint32_t*)(ptr-16))=value;
        *((uint32_t*)(ptr-12))=value;
        *((uint32_t*)(ptr-8))=value;
        *((uint32_t*)(ptr-4))=value;

        ptmp[0]=ptmp[2]=ptr[pixel_width-2];
        ptmp[1]=ptmp[3]=ptr[pixel_width-1];
        *((uint32_t*)(ptr+pixel_width))=value;
        *((uint32_t*)(ptr+pixel_width+4))=value;
        *((uint32_t*)(ptr+pixel_width+8))=value;
        *((uint32_t*)(ptr+pixel_width+12))=value;
    ...
    ptmp=(uint8_t *)&v[1];
    ptmp[0]=ptmp[2]=buf[pixel_width-2];
    ptmp[1]=ptmp[3]=buf[pixel_width-1];

    ptmp=(uint8_t *)&v[2];
    ptmp[0]=ptmp[2]=last_line[0];
    ptmp[1]=ptmp[3]=last_line[1];

    ptmp=(uint8_t *)&v[3];
    ptmp[0]=ptmp[2]=last_line[pixel_width-2];
    ptmp[1]=ptmp[3]=last_line[pixel_width-1];

    for(i=0;i<8;i++) {
        *((uint32_t*)(buf - (i + 1) * wrap -16))=v[0];
        *((uint32_t*)(buf - (i + 1) * wrap -12))=v[0];
        *((uint32_t*)(buf - (i + 1) * wrap -8))=v[0];
        *((uint32_t*)(buf - (i + 1) * wrap -4))=v[0];

        *((uint32_t*)(buf - (i + 1) * wrap + pixel_width))=v[1];
        *((uint32_t*)(buf - (i + 1) * wrap + pixel_width+4))=v[1];
        *((uint32_t*)(buf - (i + 1) * wrap + pixel_width+8))=v[1];
        *((uint32_t*)(buf - (i + 1) * wrap + pixel_width+12))=v[1];

        *((uint32_t*)(last_line + (i + 1) * wrap -16))=v[2];
        *((uint32_t*)(last_line + (i + 1) * wrap -12))=v[2];
        *((uint32_t*)(last_line + (i + 1) * wrap -8))=v[2];
        *((uint32_t*)(last_line + (i + 1) * wrap -4))=v[2];

        *((uint32_t*)(last_line + (i + 1) * wrap + pixel_width))=v[3];
        *((uint32_t*)(last_line + (i + 1) * wrap + pixel_width+4))=v[3];
        *((uint32_t*)(last_line + (i + 1) * wrap + pixel_width+8))=v[3];
        *((uint32_t*)(last_line + (i + 1) * wrap + pixel_width+12))=v[3];
...
void FUNC(ff_emulated_edge_mc_nv12)(uint8_t *buf, const uint8_t *src, int linesize, int block_w, int block_h,
                                    int src_x, int src_y, int w, int h){
    int x, y;
    int start_y, start_x, end_y, end_x;
    ...
    for(y=start_y; y<end_y; y++){
        memcpy(buf+(start_x<<1)+y*linesize,src+(start_x<<1)+y*linesize,(end_x-start_x)<<1);
//        for(x=start_x; x<end_x; x++){
//            buf[(x<<1) + y*linesize]= src[(x<<1) + y*linesize];
//            buf[(x<<1)+1 + y*linesize]= src[(x<<1)+1 + y*linesize];
//        }
    }

    //top
    for(y=0; y<start_y; y++){
        memcpy(buf+(start_x<<1)+y*linesize,buf+(start_x<<1)+start_y*linesize,(end_x-start_x)<<1);
//        for(x=start_x; x<end_x; x++){
//            buf[(x<<1) + y*linesize]= buf[(x<<1) + start_y*linesize];
//            buf[(x<<1)+1 + y*linesize]= buf[(x<<1) +1+ start_y*linesize];
//        }
    }

    //bottom
    for(y=end_y; y<block_h; y++){
        memcpy(buf+(start_x<<1)+y*linesize,buf+(start_x<<1)+(end_y-1)*linesize,(end_x-start_x)<<1);
//        for(x=start_x; x<end_x; x++){
//            buf[(x<<1) + y*linesize]= buf[(x<<1) + (end_y-1)*linesize];
//            buf[(x<<1)+1 + y*linesize]= buf[(x<<1)+1 + (end_y-1)*linesize];
//        }
    }
    ...
    for(y=0; y<block_h; y++){
       //left
        for(x=0; x<start_x; x++){
            buf[(x<<1) + y*linesize]= buf[(start_x<<1) + y*linesize];
            buf[(x<<1)+1 + y*linesize]= buf[(start_x<<1)+1 + y*linesize];
        }

       //right
        for(x=end_x; x<block_w; x++){
            buf[(x<<1) + y*linesize]= buf[((end_x - 1) <<1)+ y*linesize];
            buf[(x<<1) +1+ y*linesize]= buf[((end_x - 1)<<1) +1+ y*linesize];
        }
    }
}
...
CALL_2X_PIXELS(FUNCC(OPNAME ## _pixels8_nv12)  ,            FUNCC(OPNAME ## _pixels8)  ,           8*sizeof(pixel))\
CALL_2X_PIXELS(FUNCC(OPNAME ## _pixels8_nv12_y2)  ,         FUNCC(OPNAME ## _pixels8_y2)  ,        8*sizeof(pixel))\
CALL_2X_PIXELS(FUNCC(OPNAME ## _no_rnd_pixels8_nv12)  ,     FUNCC(OPNAME ## _no_rnd_pixels8)  ,    8*sizeof(pixel))\
CALL_2X_PIXELS(FUNCC(OPNAME ## _no_rnd_pixels8_nv12_y2)  ,  FUNCC(OPNAME ## _no_rnd_pixels8_y2)  , 8*sizeof(pixel))\
...
static void FUNCC(put_pixels8_nv12_x2)(uint8_t *block, const uint8_t *pixels, int line_size, int h){
    int i,j;
...
static void FUNCC(avg_pixels8_nv12_x2)(uint8_t *block, const uint8_t *pixels, int line_size, int h){
    int i,j;
    uint8_t* psrc=(uint8_t *)pixels;
    uint8_t* pdes=block;
    for(j=0;j<h;j++)
    {
        for(i=0;i<16;i++)
        {
            pdes[i]=((psrc[i+2]+psrc[i]+1)/2+pdes[i]+1)>>1;
        }
        pdes+=line_size;
        psrc+=line_size;
    }
...
static void FUNCC(put_no_rnd_pixels8_nv12_x2)(uint8_t *block, const uint8_t *pixels, int line_size, int h){
    int i,j;
    uint8_t* psrc=(uint8_t *)pixels;
    uint8_t* pdes=block;
    for(j=0;j<h;j++)
    {
        for(i=0;i<16;i++)
        {
            pdes[i]=(psrc[i+2]+psrc[i])>>1;
        }
        pdes+=line_size;
        psrc+=line_size;
    }
}
static void FUNCC(avg_no_rnd_pixels8_nv12_x2)(uint8_t *block, const uint8_t *pixels, int line_size, int h){
    int i,j;
    uint8_t* psrc=(uint8_t *)pixels;
    uint8_t* pdes=block;
    ...
    for(j=0;j<h;j++)
    {
        for(i=0;i<16;i++)
        {
            pdes[i]=((psrc[i+2]+psrc[i])/2+pdes[i]+1)>>1;
        }
        pdes+=line_size;
        psrc+=line_size;
    }
}
static void FUNCC(put_pixels8_nv12_xy2)(uint8_t *block, const uint8_t *pixels, int line_size, int h){
    int i,j;
    uint8_t* psrc=(uint8_t *)pixels,*psrc1=(uint8_t *)((uint8_t *)pixels+line_size);
    uint8_t* pdes=block;
    ...
    for(j=0;j<h;j++)
    {
        for(i=0;i<16;i++)
        {
            pdes[i]=(psrc[2+i]+psrc[i]+psrc1[i+2]+psrc1[i]+2)>>2;
        }
        pdes+=line_size;
        psrc+=line_size;
        psrc1+=line_size;
...
static void FUNCC(avg_pixels8_nv12_xy2)(uint8_t *block, const uint8_t *pixels, int line_size, int h){
    int i,j;
    uint8_t* psrc=(uint8_t *)pixels,*psrc1=(uint8_t *)((uint8_t *)pixels+line_size);
    uint8_t* pdes=block;
    ...
    for(j=0;j<h;j++)
    {
        for(i=0;i<16;i++)
        {
            pdes[i]=(((psrc[2+i]+psrc[i]+psrc1[i+2]+psrc1[i]+2)>>2)+pdes[i]+1)>>1;
        }
        pdes+=line_size;
        psrc+=line_size;
        psrc1+=line_size;
...
static void FUNCC(put_no_rnd_pixels8_nv12_xy2)(uint8_t *block, const uint8_t *pixels, int line_size, int h){
    int i,j;
    uint8_t* psrc=(uint8_t *)pixels,*psrc1=(uint8_t *)((uint8_t *)pixels+line_size);
    uint8_t* pdes=block;
    ...
    for(j=0;j<h;j++)
    {
        for(i=0;i<16;i++)
        {
            pdes[i]=(psrc[i+2]+psrc[i]+psrc1[i+2]+psrc1[i]+1)>>2;
        }
        pdes+=line_size;
        psrc+=line_size;
        psrc1+=line_size;
...
static void FUNCC(avg_no_rnd_pixels8_nv12_xy2)(uint8_t *block, const uint8_t *pixels, int line_size, int h){
    int i,j;
    uint8_t* psrc=(uint8_t *)pixels,*psrc1=(uint8_t *)((uint8_t *)pixels+line_size);
    uint8_t* pdes=block;
    ...
    for(j=0;j<h;j++)
    {
        for(i=0;i<16;i++)
        {
            pdes[i]=(((psrc[i+2]+psrc[i]+psrc1[i+2]+psrc1[i]+1)>>2)+pdes[i])>>1;
        }
        pdes+=line_size;
        psrc+=line_size;
        psrc1+=line_size;
...
\
static void FUNCC(OPNAME ## h264_chroma_mc2_nv12)(uint8_t *dst/*align 8*/, uint8_t *src/*align 1*/, int stride, int h, int x, int y){\
    const int A=(8-x)*(8-y);\
    const int B=(  x)*(8-y);\
    const int C=(8-x)*(  y);\
    const int D=(  x)*(  y);\
...
\
    if(D){\
        for(i=0; i<h; i++){\
            OP(dst[0], (A*src[0] + B*src[2] + C*src[stride+0] + D*src[stride+2]));\
            OP(dst[1], (A*src[2] + B*src[4] + C*src[stride+2] + D*src[stride+4]));\
            dst+= stride;\
            src+= stride;\
        }\
    }else{\
...
\
static void FUNCC(OPNAME ## h264_chroma_mc4_nv12)(uint8_t *dst/*align 8*/, uint8_t *src/*align 1*/, int stride, int h, int x, int y){\
    const int A=(8-x)*(8-y);\
    const int B=(  x)*(8-y);\
    const int C=(8-x)*(  y);\
    const int D=(  x)*(  y);\
...
\
    if(D){\
        for(i=0; i<h; i++){\
            OP(dst[0], (A*src[0] + B*src[2] + C*src[stride+0] + D*src[stride+2]));\
            OP(dst[1], (A*src[2] + B*src[4] + C*src[stride+2] + D*src[stride+4]));\
            OP(dst[2], (A*src[4] + B*src[6] + C*src[stride+4] + D*src[stride+6]));\
            OP(dst[3], (A*src[6] + B*src[8] + C*src[stride+6] + D*src[stride+8]));\
            dst+= stride;\
            src+= stride;\
        }\
    }else{\
        ...
        for(i=0; i<h; i++){\
            OP(dst[0], (A*src[0] + E*src[step+0]));\
            OP(dst[1], (A*src[2] + E*src[step+2]));\
            OP(dst[2], (A*src[4] + E*src[step+4]));\
            OP(dst[3], (A*src[6] + E*src[step+6]));\
            dst+= stride;\
            src+= stride;\
        }\
    }\
...
\
static void FUNCC(OPNAME ## h264_chroma_mc8_nv12)(uint8_t *dst/*align 8*/, uint8_t *src/*align 1*/, int stride, int h, int x, int y){\
    const int A=(8-x)*(8-y);\
    const int B=(  x)*(8-y);\
    const int C=(8-x)*(  y);\
    const int D=(  x)*(  y);\
...
\
    if(D){\
        for(i=0; i<h; i++){\
            ...
            OP(dst[2], (A*src[4] + B*src[6] + C*src[stride+4] + D*src[stride+6]));\
            OP(dst[3], (A*src[6] + B*src[8] + C*src[stride+6] + D*src[stride+8]));\
            OP(dst[4], (A*src[8] + B*src[10] + C*src[stride+8] + D*src[stride+10]));\
            OP(dst[5], (A*src[10] + B*src[12] + C*src[stride+10] + D*src[stride+12]));\
            OP(dst[6], (A*src[12] + B*src[14] + C*src[stride+12] + D*src[stride+14]));\
            OP(dst[7], (A*src[14] + B*src[16] + C*src[stride+14] + D*src[stride+16]));\
            dst+= stride;\
            src+= stride;\
        }\
    }else{\
        ...
        for(i=0; i<h; i++){\
            ...
            OP(dst[2], (A*src[4] + E*src[step+4]));\
            OP(dst[3], (A*src[6] + E*src[step+6]));\
            OP(dst[4], (A*src[8] + E*src[step+8]));\
            OP(dst[5], (A*src[10] + E*src[step+10]));\
            OP(dst[6], (A*src[12] + E*src[step+12]));\
            OP(dst[7], (A*src[14] + E*src[step+14]));\
            dst+= stride;\
            src+= stride;\
        }\
    }\
...
void FUNCC(ff_put_vc1_pixels8x8)(uint8_t *dst, const uint8_t *src, int stride, int h) {
    FUNCC(put_pixels8)(dst, src, stride, 8);
}
void FUNCC(ff_avg_vc1_pixels8x8)(uint8_t *dst, const uint8_t *src, int stride, int h) {
    FUNCC(avg_pixels8)(dst, src, stride, 8);
}
void FUNCC(ff_put_vc1_pixels16x16)(uint8_t *dst, const uint8_t *src, int stride, int h) {
    FUNCC(put_pixels16)(dst, src, stride, 16);
}
void FUNCC(ff_avg_vc1_pixels16x16)(uint8_t *dst, const uint8_t *src, int stride, int h) {
    FUNCC(avg_pixels16)(dst, src, stride, 16);
}
