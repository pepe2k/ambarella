...
void mpeg4_pred_ac_amba(MpegEncContext * s, DCTELEM *block, int n,
                   int dir)
{
    int i;
    int16_t *ac_val, *ac_val1;
    int8_t * const qscale_table= s->current_picture_ptr->qscale_table;
    ...
    if (s->ac_pred) {
        if (dir == 0) {
            ...
            if(s->mb_x==0 || s->qscale == qscale_table[xy] || n==1 || n==3){
                /* same qscale */
                for(i=1;i<8;i++) {
                    block[s->dsp.p_idct_permutation[i<<3]] += ac_val[i];
                }
            }else{
                /* different qscale, we must rescale */
                for(i=1;i<8;i++) {
                    block[s->dsp.p_idct_permutation[i<<3]] += ROUNDED_DIV(ac_val[i]*qscale_table[xy], s->qscale);
                }
            }
        } else {
            const int xy= s->mb_x + s->mb_y*s->mb_stride - s->mb_stride;
            ...
            if(s->mb_y==0 || s->qscale == qscale_table[xy] || n==2 || n==3){
                /* same qscale */
                for(i=1;i<8;i++) {
                    block[s->dsp.p_idct_permutation[i]] += ac_val[i + 8];
                }
            }else{
                /* different qscale, we must rescale */
                for(i=1;i<8;i++) {
                    block[s->dsp.p_idct_permutation[i]] += ROUNDED_DIV(ac_val[i + 8]*qscale_table[xy], s->qscale);
                }
            }
        }
    }
    /* left copy */
    for(i=1;i<8;i++)
        ac_val1[i    ] = block[s->dsp.p_idct_permutation[i<<3]];

    /* top copy */
    for(i=1;i<8;i++)
        ac_val1[8 + i] = block[s->dsp.p_idct_permutation[i   ]];
...
int mpeg4_decode_video_packet_header(MpegEncContext *s)
{
    ...
    if(header_extension){
        ...
        av_log(NULL, AV_LOG_DEBUG, "time_increment=%d\n", time_increment);
...
static int decode_vol_header(MpegEncContext *s, GetBitContext *gb){
    ...
    if (s->shape != BIN_ONLY_SHAPE) {
        ...
        if (!s->avctx->mpeg4_interlaced) {
            s->avctx->mpeg4_interlaced = (s->progressive_sequence ? 0 : 1);
        }
        s->interlaced_dct=0;
...
no_cplx_est:
        ...
        if (s->scalability) {
            ...
            av_log(NULL, AV_LOG_DEBUG, "ref_layer_id=%d, ref_layer_sampling_dir=%dn", ref_layer_id, ref_layer_sampling_dir);
...
static int decode_user_data(MpegEncContext *s, GetBitContext *gb){
    ...
    if(e>=2){
        ...
        av_log(s->avctx, AV_LOG_ERROR, "***Debug, DivX dectected in userdata, version=%d,build=%d.\n",ver,build);
    }
    ...
    if(e!=4){
        if(strcmp(buf, "ffmpeg")==0){
            s->lavc_build= 4600;
            av_log(s->avctx, AV_LOG_ERROR, "***Debug, e!=4, FFmpeg dectected 'ffmpeg' in userdata, lavc_build=4600.\n");
        }
    }
    if(e==4){
        s->lavc_build= build;
        av_log(s->avctx, AV_LOG_ERROR, "***Debug, e==4, FFmpeg dectected in userdata, build=%d.\n",build);
    }
...
//FIXME reduced res stuff
     ...
     if (s->shape != RECT_SHAPE) {
         if (s->vol_sprite_usage != 1 || s->pict_type != FF_I_TYPE) {
             ...
             av_log(NULL, AV_LOG_DEBUG, "width=%d, height=%d, hor_spat_ref=%d, ver_spat_ref=%d\n", width, height, hor_spat_ref, ver_spat_ref);
         }
...
int ff_mpeg4_decode_picture_header(MpegEncContext * s, GetBitContext *gb)
{
    ...
    for(;;) {
        ...
        av_log(s->avctx, AV_LOG_ERROR, "startcode=[%8x]\n", startcode);
...
int ff_mpeg4_decode_picture_header_parser(MpegEncContext * s, GetBitContext *gb)
{
    int startcode, v;
