                          ...
                          SBRData *ch_data, const int e_a[2])
{
    ...
    static const float limgain[4] = { 0.70795, 1.0, 1.41254, 10000000000.0f };
...
void ff_sbr_apply(AACContext *ac, SpectralBandReplication *sbr, int id_aac,
                  float* L, float* R)
{
    //BUG1114 & 1266: when ext_sample_rate = sample_rate, it is need to do desample.
    //Modify: add one more judgement : if ext_sample_rate <= sample_rate, set : desampled = 1. by cxing 2011-08-11
    //Original: int downsampled = ac->m4ac.ext_sample_rate < sbr->sample_rate;
    int downsampled = (ac->m4ac.ext_sample_rate < sbr->sample_rate ? 1 : ac->m4ac.ext_sample_rate <= ac->m4ac.sample_rate);
    int ch;
    ...
    for (ch = 0; ch < nch; ch++) {
        ...
        sbr_qmf_analysis(&ac->dsp, &sbr->mdct_ana, ch ? R : L, sbr->data[ch].analysis_filterbank_samples,
                         ...
                         sbr->data[ch].W);
        sbr_lf_gen(ac, sbr, sbr->X_low, (const float (*)[32][32][2])sbr->data[ch].W);
        if (sbr->start) {
            sbr_hf_inverse_filter(sbr->alpha0, sbr->alpha1, (const float (*)[40][2])sbr->X_low, sbr->k[0]);
            ...
            sbr_hf_assemble(sbr->data[ch].Y, (const float (*)[40][2])sbr->X_high, sbr, &sbr->data[ch],
                            sbr->data[ch].e_a);
        }

        /* synthesis */
        sbr_x_gen(sbr, sbr->X[ch], (const float (*)[40][2])sbr->X_low, (const float (*)[38][64][2])sbr->data[ch].Y, ch);
    }
