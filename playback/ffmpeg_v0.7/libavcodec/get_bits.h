...
static inline int get_xbits(GetBitContext *s, int n){
    ...
    LAST_SKIP_BITS(re, s, n)
    CLOSE_READER(re, s)
    ret= (NEG_USR32(sign ^ cache, n) ^ sign) - sign;
    av_log(NULL,AV_LOG_ERROR,"get_xbits,n=%d,ret=%d.\n",n,ret);
    trap_getbit();
    return ret;
}
...
static inline int get_sbits(GetBitContext *s, int n){
    ...
    LAST_SKIP_BITS(re, s, n)
    CLOSE_READER(re, s)
    av_log(NULL,AV_LOG_ERROR,"get_sbits,n=%d,ret=%d.\n",n,tmp);
    trap_getbit();
    return tmp;
}
...
static inline unsigned int get_bits(GetBitContext *s, int n){
    ...
    LAST_SKIP_BITS(re, s, n)
    CLOSE_READER(re, s)
    av_log(NULL,AV_LOG_ERROR,"get_bits,n=%d,ret=%d.\n",n,tmp);
    trap_getbit();
    return tmp;
}
...
static inline unsigned int show_bits(GetBitContext *s, int n){
    ...
    tmp= SHOW_UBITS(re, s, n);
//    CLOSE_READER(re, s)
    av_log(NULL,AV_LOG_ERROR,"show_bits,n=%d,ret=%d.\n",n,tmp);
    trap_getbit();
    return tmp;
}
...
static inline void skip_bits(GetBitContext *s, int n){
 //Note gcc seems to optimize this to s->index+=n for the ALT_READER :))
    ...
    LAST_SKIP_BITS(re, s, n)
    CLOSE_READER(re, s)
    av_log(NULL,AV_LOG_ERROR,"skip_bits,n=%d.\n",n);
    trap_getbit();
}
...
static inline unsigned int get_bits1(GetBitContext *s){
    ...
    index++;
    s->index= index;
    av_log(NULL,AV_LOG_ERROR,"get_bits1,ret=%d.\n",result);
    trap_getbit();
    return result;
#else
    int result;
...
static inline unsigned int show_bits1(GetBitContext *s){
    int result;
    result=show_bits(s, 1);
    av_log(NULL,AV_LOG_ERROR,"show_bits1,ret=%d.\n",result);
    trap_getbit();
    return result;
}
...
static inline void skip_bits1(GetBitContext *s){
    skip_bits(s, 1);
    av_log(NULL,AV_LOG_ERROR,"skip_bits1.\n");
    trap_getbit();
}
...
#ifdef ALT_BITSTREAM_READER_LE
        int ret= get_bits(s, 16);
        ret=ret | (get_bits(s, n-16) << 16);
        av_log(NULL,AV_LOG_ERROR,"get_bits_long 1,n=%d,ret=%d.\n",n,ret);
        trap_getbit();

        return ret ;
#else
        int ret= get_bits(s, 16) << (n-16);
        ret=ret | get_bits(s, n-16);
        av_log(NULL,AV_LOG_ERROR,"get_bits_long 2,n=%d,ret=%d.\n",n,ret);
        trap_getbit();
 ...
 */
static inline int get_sbits_long(GetBitContext *s, int n) {
    int ret=sign_extend(get_bits_long(s, n), n);
    av_log(NULL,AV_LOG_ERROR,"get_sbits_long,n=%d,ret=%d.\n",n,ret);
    trap_getbit();
...
static inline unsigned int show_bits_long(GetBitContext *s, int n){
    ...
    {
        ret=show_bits(s, n);
        av_log(NULL,AV_LOG_ERROR,"show_bits_long 1,n=%d,ret=%d.\n",n,ret);
        trap_getbit();
        return ret;
    }
    else{
        GetBitContext gb= *s;
        ret=get_bits_long(&gb, n);
        av_log(NULL,AV_LOG_ERROR,"show_bits_long 2,n=%d,ret=%d.\n",n,ret);
        trap_getbit();        
        return ret;
    }
}
                   ...
                   const uint8_t *buffer, int bit_size)
{
    ...
    skip_bits_long(s, 0);
#endif
    av_log(NULL,AV_LOG_ERROR,"init_get_bits,bit_size=%d.\n",bit_size);
    trap_getbit();        
...
static inline void align_get_bits(GetBitContext *s)
{
    ...
    av_log(NULL,AV_LOG_ERROR,"align_get_bits,skip n=%d.\n",n);
    trap_getbit();   
}
                                  ...
                                  int bits, int max_depth)
{
    ...
    CLOSE_READER(re, s)
    av_log(NULL,AV_LOG_ERROR,"get_vlc2,return=%d.\n",code);
    trap_getbit();  
    return code;
}
