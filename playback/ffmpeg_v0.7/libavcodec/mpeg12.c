...
static enum PixelFormat mpeg_get_pixelformat(AVCodecContext *avctx){
    ...
    av_log(NULL,AV_LOG_ERROR,"!!!![special pixformat]: detected PIX_FMT_YUV422P format in mpeg1/mpeg2.\n");
    #endif
    ...
    {
    #ifdef __log_special_case__
    av_log(NULL,AV_LOG_ERROR,"!!!![special pixformat]: detected PIX_FMT_YUV444P format in mpeg1/mpeg2.\n");
    #endif
...
static void mpeg_decode_gop(AVCodecContext *avctx,
    ...
    av_log(NULL, AV_LOG_DEBUG, "drop_frame_flag=%d\n", drop_frame_flag);
                             ...
                             AVPacket *avpkt)
{
    ...
    log_openfile_text(log_fd_dsp_text,"mpeg2_text");
#endif

    ret=decode_chunks(avctx, picture, data_size, buf, buf_size);
    
    #ifdef __log_dump_data__
    AVFrame* pdecpic=(AVFrame*)s2->current_picture_ptr;
    if(pdecpic->data[0])
    {
        int ret=0;
    //        av_log(NULL,AV_LOG_WARNING,"dump only pictuce data,s->flags&CODEC_FLAG_EMU_EDGE=%d,avctx->width=%d,avctx->height=%d, pict->linesize[0]=%d,pict->linesize[1]=%d.\n",s->flags&CODEC_FLAG_EMU_EDGE,avctx->width,avctx->height,pdecpic->linesize[0],pdecpic->linesize[1]);
        ...
        if(s2->flags&CODEC_FLAG_EMU_EDGE || !dump_config_extedge)
        {
