...
/**
 * @file libavcodec/h263_nv12.c
 * h263/mpeg4 codec (NV12).
 */
...
static int h263_decode_gob_header(MpegEncContext *s)
{
    ...
    av_log(NULL, AV_LOG_DEBUG, "gfid=%u\n", gfid);
 ...
 * @return <0 if an error occurred
 */
static int mpeg4_decode_partitioned_mb_nv12(MpegEncContext *s, DCTELEM block[6][64])
{
    int cbp, mb_type;
    const int xy= s->mb_x + s->mb_y*s->mb_stride;
...
//    av_log(NULL,AV_LOG_ERROR,"mbtype x=%d,y=%d, s->mb_intra=%d.\n",s->mb_x,s->mb_y,s->mb_intra);
//    av_log(NULL,AV_LOG_ERROR,"s->current_picture_ptr->mb_type[xy]=%x.\n",s->current_picture_ptr->mb_type[xy]);
...
no_cplx_est:
        ...
        if (s->scalability) {
            ...
            av_log(NULL, AV_LOG_DEBUG, "ref_layer_id=%d, ref_layer_sampling_dir=%d\n", ref_layer_id, ref_layer_sampling_dir);
...
static int decode_vop_header_nv12(MpegEncContext *s, GetBitContext *gb){
    ...
    s->partitioned_frame= s->data_partitioning && s->pict_type!=FF_B_TYPE;
    if(s->partitioned_frame)
        s->decode_mb= mpeg4_decode_partitioned_mb_nv12;
    else
...
//                av_log(NULL, AV_LOG_ERROR, "****end of stream,gb->size_in_bits=%d, s->divx_version=%d,s->xvid_build=%d.\n",gb->size_in_bits,s->divx_version,s->xvid_build);
    ...
    return decode_vop_header_nv12(s, gb);
}
#if 0
...
int ff_flv_decode_picture_header(MpegEncContext *s)
{
    ...
    if(av_image_check_size(width, height, 0, r->s.avctx))
        return -1;
    s->width = width;
    s->height = height;
