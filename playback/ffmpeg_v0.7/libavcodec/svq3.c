                             ...
                             AVPacket *avpkt)
{
    ...
    if (s->pict_type == FF_B_TYPE || s->low_delay || avctx->codec->flush_flag) {//fix bug#2148, should not use LAST_picture->reordered_opaque after seek, because pts leap after seeking will cause dsp buffer release delay and request buffer blocked, roy 12.03.31
        av_log(NULL, AV_LOG_DEBUG, "svq3 CUR_pict_type=%d, flush_flag=%d, rtn CURR_picture->reordered_opaque=%lld\n", s->pict_type,avctx->codec->flush_flag,((AVFrame *) &s->current_picture)->reordered_opaque);
        if(avctx->codec->flush_flag)
        {
            avctx->codec->flush_flag = 0;
        }
        *(AVFrame *) data = *(AVFrame *) &s->current_picture;
    } else {
        *(AVFrame *) data = *(AVFrame *) &s->last_picture;
        av_log(NULL, AV_LOG_DEBUG, "svq3 CUR_pict_type=%d, flush_flag=%d, rtn LAST_picture->reordered_opaque=%lld\n", s->pict_type,avctx->codec->flush_flag,((AVFrame *) &s->last_picture)->reordered_opaque);
    }
...
static void svq3_flush(AVCodecContext *avctx)
{
    avctx->codec->flush_flag= 1;
    av_log(NULL, AV_LOG_DEBUG, "svq3_flush done, flush_flag=%d\n", avctx->codec->flush_flag);
}
