...
static void srt_style_apply(SRTContext *s, const char *style)
{
    ...
    if (st) {
        ...
        /*if (st->font_name && strcmp(st->font_name, ASS_DEFAULT_FONT) ||
            ...
            c != ASS_DEFAULT_COLOR) {*/
        if ((st->font_name && strcmp(st->font_name, ASS_DEFAULT_FONT)) ||//NOTE: parentheses
            (st->font_size != ASS_DEFAULT_FONT_SIZE) ||
            (c != ASS_DEFAULT_COLOR)) {
            srt_print(s, "<font");
            if (st->font_name && strcmp(st->font_name, ASS_DEFAULT_FONT))
...
//                          (c & 0xFF0000) >> 16 | c & 0xFF00 | (c & 0xFF) << 16);
                          ((c & 0xFF0000) >> 16) | (c & 0xFF00) | ((c & 0xFF) << 16));//NOTE: parentheses
            srt_print(s, ">");
            srt_stack_push(s, 'f');
        }
...
//            (color & 0xFF0000) >> 16 | color & 0xFF00 | (color & 0xFF) << 16);
              ((color & 0xFF0000) >> 16) | (color & 0xFF00) | ((color & 0xFF) << 16));//NOTE: parentheses
}
