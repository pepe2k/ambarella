...
static void hybrid_analysis(float out[91][32][2], float in[5][44][2], float L[2][38][64], int is34, int len)
{
    ...
    if (is34) {
        hybrid4_8_12_cx(in[0], out,    (const float (*)[7][2])f34_0_12, 12, len);
        hybrid4_8_12_cx(in[1], out+12, (const float (*)[7][2])f34_1_8,   8, len);
        hybrid4_8_12_cx(in[2], out+20, (const float (*)[7][2])f34_2_4,   4, len);
        hybrid4_8_12_cx(in[3], out+24, (const float (*)[7][2])f34_2_4,   4, len);
        hybrid4_8_12_cx(in[4], out+28, (const float (*)[7][2])f34_2_4,   4, len);
        for (i = 0; i < 59; i++) {
            for (j = 0; j < len; j++) {
        ...
        hybrid6_cx(in[0], out, (const float (*)[7][2])f20_0_8, len);
...
static void stereo_processing(PSContext *ps, float (*l)[32][2], float (*r)[32][2], int is34)
{
    ...
    const int8_t *k_to_i = is34 ? k_to_i_34 : k_to_i_20;
    const float (*H_LUT)[8][4] = (const float (*)[8][4])((PS_BASELINE || ps->icc_mode < 3) ? HA : HB);
