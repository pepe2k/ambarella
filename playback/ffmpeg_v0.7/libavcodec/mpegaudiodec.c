...
static int mp_decode_layer3(MPADecodeContext *s)
{
    ...
    av_log(NULL, AV_LOG_DEBUG, "private_bits=%d\n", private_bits);
...
#if CONFIG_OPENCOREMP3_DECODER
#include <mp3/mp3_dec.h>
static av_cold int Decode_init_pv(AVCodecContext * avctx)
{
    MPADecodeContext *s = avctx->priv_data;
    static int init=0;
    int i, j, k;
...
static int Decode_frame_pv(AVCodecContext * avctx,
                        void *data, int *data_size,
                        AVPacket *avpkt)
{
    const uint8_t *buf = avpkt->data;
    ...
    out_size = Decode_frame_mp3(s->state, out_samples, buf, buf_size,data_size);
    if(out_size>=0){
...
static int Decode_close_pv(AVCodecContext *avctx)
{
    MPADecodeContext *s = avctx->priv_data;

    Decoder_exit_mp3(s->state);
    return 0;
}
#endif /* CONFIG_OPENCOREMP3_DECODER */
...
#if CONFIG_OPENCOREMP3_DECODER
#include <mp3/mp3_dec.h>
AVCodec ff_opencoremp3_decoder =
{
    "opencoremp3",
    AVMEDIA_TYPE_AUDIO,
    CODEC_ID_MP3PV,
