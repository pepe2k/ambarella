                               ...
                               int motion_x, int motion_y, int h)
{
    ...
    ptr = ref_picture[1] + offset;
    if(emu){
        s->dsp.emulated_edge_mc_nv12(s->edge_emu_buffer, ptr, s->uvlinesize, 9, 9,
                         src_x, src_y, s->h_edge_pos>>1, s->v_edge_pos>>1);
        ptr= s->edge_emu_buffer;
    }
    pix_op_nv12[1][dxy](dest_cb, ptr, uvlinesize, h >> 1);
...
void wmv2_tran_block(Wmv2Context_amba*w, DCTELEM *block1,int n)
{
  ...
  if (s->block_last_index[n] >= 0) {
    ...
    case 0:
        ...
        break;
    case 1:
        ff_simple_idct84_wmv2(block1);
        ff_simple_idct84_wmv2_des(w->abt_block2[n],block1+32);
        s->dsp.clear_block(w->abt_block2[n]);
        break;
    case 2:
        ff_simple_idct48_wmv2(block1);
        ff_simple_idct48_wmv2_des(w->abt_block2[n],block1+4);
        s->dsp.clear_block(w->abt_block2[n]);
        break;
    default:
        av_log(s->avctx, AV_LOG_ERROR, "internal error in WMV2 abt\n");
...
/*
void ff_wmv2_tran_mb(MpegEncContext *s, DCTELEM block1[6][64]){
    Wmv2Context_amba * const w= (Wmv2Context_amba*)s;

    wmv2_tran_block(w, block1[0], 0);
    wmv2_tran_block(w, block1[1], 1);
    wmv2_tran_block(w, block1[2], 2);
    wmv2_tran_block(w, block1[3], 3);

    if(s->flags&CODEC_FLAG_GRAY) return;

    wmv2_tran_block(w, block1[4], 4);
    wmv2_tran_block(w, block1[5], 5);
}*/
