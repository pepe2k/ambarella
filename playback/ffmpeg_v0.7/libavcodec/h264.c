...
static int decode_slice_header(H264Context *h, H264Context *h0){
    ...
    if (!s->context_initialized) {
        ...
        if(h->sps.timing_info_present_flag){
            ...
            int64_t num= h->sps.num_units_in_tick;
            if(h->x264_build < 44U)
                den *= 2;

            // ref doc:
            // ISO-IEC 14496-10 E-10 && Table E-6
            int div = 1;
            if (h->sps.fixed_frame_rate_flag) {
                div = 1;
            } else {
                if (h->sps.pic_struct_present_flag)
                {
...
//                       || get_bits_count(&s->gb) <  s->gb.size_in_bits && s->avctx->error_recognition < FF_ER_AGGRESSIVE) {
                       || (get_bits_count(&s->gb) <  s->gb.size_in_bits && s->avctx->error_recognition < FF_ER_AGGRESSIVE)) {//NOTE: parentheses
                        ff_er_add_slice(s, s->resync_mb_x, s->resync_mb_y, s->mb_x-1, s->mb_y, (AC_END|DC_END|MV_END)&part_mask);
