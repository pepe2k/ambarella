...
static MyMap s_map;
G726LiveSource* G726LiveSource::createNew(UsageEnvironment& env,char *const streamName) {
    return new G726LiveSource(env,streamName);
}

void signalNewFrameDataG726(void *device) {
  G726LiveSource *ourDevice = (G726LiveSource*)device;
...
::G726LiveSource(UsageEnvironment& env, char *const streamName)
  ...
  if(s_map.get_and_inc(streamName_) == 0){
    ...
    RtspService::instance()->registerSigFrameDataAudio(streamName_,signalNewFrameDataG726,this);
    printf("G726LiveSource::G726LiveSource , streamName [%s]\n",streamName_);
 }
}

G726LiveSource::~G726LiveSource() {
...
void G726LiveSource::deliverFrame0(void* clientData){
     ((G726LiveSource*)clientData)->deliverFrame();
}

void G726LiveSource::deliverFrame(){\
     if (!isCurrentlyAwaitingData()) return; // we're not ready for the data yet
     doGetNextFrame();
}

void G726LiveSource::doGetNextFrame() {
  int isLive;
  if(RtspService::instance()->getAudioFrame(streamName_,tmp_data_,isLive) < 0){
    printf("G726LiveSource::doGetNextFrame -- handleClosure\n");
    handleClosure(this);
    return;
  }
  if(!tmp_data_){
  ...
  if(!tmp_data_->len){
    printf("G726LiveSource::doGetNextFrame -- data_len == 0, unexpected\n");
