...
class IDarwinInjectorEventHandler{
public:
	virtual ~IDarwinInjectorEventHandler(){}
	virtual void onInjectorPeerClose() = 0;
};
...
public:
  ...
  //setEventHandler() should be called before setDestination
  //   if "handler" needed
  void setEventHandler(IDarwinInjectorEventHandler *handler){
      fHandler = handler;
  }
  void onPeerClose(){
	  if(fHandler){
		  fHandler->onInjectorPeerClose();
	  }
  }
...
private:
  ...
  int fResultCode;
  char* fResultString;
  IDarwinInjectorEventHandler *fHandler;
};
